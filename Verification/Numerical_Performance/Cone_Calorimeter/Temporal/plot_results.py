...
results_file_path1 = os.path.join(SCRIPT_DIR, 'CC_time_step_convergence.png')
results_file_path2 = os.path.join(SCRIPT_DIR, 'CC_time_step_error.png')
results_file_path3 = os.path.join(SCRIPT_DIR, 'CC_time_step_CPU.png')
...
log_dt = np.log10(dt_values[1:])
log_err = np.log10(errors[1:])
slope, intercept, r_value, p_value, std_err = linregress(log_dt, log_err)

# Plot fit line
fit_line = 10**(slope * np.log10(dt_values) + intercept)
