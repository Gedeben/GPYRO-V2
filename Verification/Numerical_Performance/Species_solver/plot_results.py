...
# Load data
try:
    Gpyrodt100=pd.read_csv("dt_100s/ATG_PMMA_summary_01_0001.csv")
    Gpyrodt50=pd.read_csv("dt_50s/ATG_PMMA_summary_01_0001.csv")
    Gpyrodt10=pd.read_csv("dt_10s/ATG_PMMA_summary_01_0001.csv")
    Gpyrodt1=pd.read_csv("dt_1s/ATG_PMMA_summary_01_0001.csv")
    Gpyrodt05=pd.read_csv("dt_05s/ATG_PMMA_summary_01_0001.csv")
    Gpyrodt01=pd.read_csv("dt_01s/ATG_PMMA_summary_01_0001.csv")
    Gpyrodt001=pd.read_csv("dt_001s/ATG_PMMA_summary_01_0001.csv")
    Gpyrodt005=pd.read_csv("dt_005s/ATG_PMMA_summary_01_0001.csv")
    Gpyrodt0001=pd.read_csv("dt_0001s/ATG_PMMA_summary_01_0001.csv")
    #Gpyrodt00001=pd.read_csv("dt_00001s/ATG_PMMA_summary_01_0001.csv")
...
style_traces = {
    ...
    "dt05":    {'label':"dt=0.5s"  , "color": "orange", "linestyle": "--", "linewidth": 3},
    "dt005":  {'label':"dt=0.05s" , "color": "gray"  , "linestyle": "-.", "linewidth": 3},
    "dt001": {'label':"dt=0.01s", "color": "brown" , "linestyle": ":" , "linewidth": 3},
...
# Premier graphique
plt.plot(Gpyrodt100['t'].values, Gpyrodt100['002_MLR( 0.0000_ 0.0000_ 0.0000)'].values, **style_traces["dt100"])
plt.plot(Gpyrodt50['t'].values, Gpyrodt50['002_MLR( 0.0000_ 0.0000_ 0.0000)'].values, **style_traces["dt50"])
plt.plot(Gpyrodt10['t'].values, Gpyrodt10['002_MLR( 0.0000_ 0.0000_ 0.0000)'].values, **style_traces["dt10"])
plt.plot(Gpyrodt1['t'].values, Gpyrodt1['002_MLR( 0.0000_ 0.0000_ 0.0000)'].values,  **style_traces["dt1"])
plt.plot(Gpyrodt05['t'].values, Gpyrodt05['002_MLR( 0.0000_ 0.0000_ 0.0000)'].values, **style_traces["dt05"])
plt.plot(Gpyrodt01['t'].values, Gpyrodt01['002_MLR( 0.0000_ 0.0000_ 0.0000)'].values, **style_traces["dt01"])
plt.plot(Gpyrodt005['t'].values, Gpyrodt005['002_MLR( 0.0000_ 0.0000_ 0.0000)'].values, **style_traces["dt005"])
plt.plot(Gpyrodt001['t'].values, Gpyrodt001['002_MLR( 0.0000_ 0.0000_ 0.0000)'].values, **style_traces["dt001"])
#plt.plot(Gpyrodt0001['t'].values, Gpyrodt0001['002_MLR( 0.0000_ 0.0000_ 0.0000)'].values, **style_traces["dt0001"])
...
an_MLR= Gpyrodt0001['002_MLR( 0.0000_ 0.0000_ 0.0000)'].values


for dt, df in sorted(cases.items(), reverse=True):
    t = df['t'].values
    mlr = df['002_MLR( 0.0000_ 0.0000_ 0.0000)'].values
    errorL2 = L2(t, mlr, an_t, an_MLR)
    errorL1 = L1(t, mlr, an_t, an_MLR)
    errorLinf = Linf(t, mlr, an_t, an_MLR)
    dt_values.append(dt)
    errorsL2.append(errorL2)
    errorsL1.append(errorL1)
    errorsLinf.append(errorLinf)


# Conversion en log
log_dt = np.log10(dt_values)
log_err = np.log10(errorsL2)
...
plt.figure()
plt.loglog(dt_values, errorsL2, marker='o', linestyle='-', linewidth=4, markersize=15, color='k', label=r'Mesured $L^2$ error')
#plt.loglog(dt_values, errorsL1, marker='o', linestyle='-', linewidth=4, markersize=15, color='b', label=r'$L_1$ error')
#plt.loglog(dt_values, errorsLinf, marker='o', linestyle='-', linewidth=4, markersize=15, color='g', label=r'$L_\infty$ error')
...
timing_files = {
    100: 'dt_100s/ATG_PMMA_timing_01.csv',
    50:  'dt_50s/ATG_PMMA_timing_01.csv',
    10:  'dt_10s/ATG_PMMA_timing_01.csv',
    1:  'dt_1s/ATG_PMMA_timing_01.csv',
    0.5: 'dt_05s/ATG_PMMA_timing_01.csv',
    0.1: 'dt_01s/ATG_PMMA_timing_01.csv',
    0.05: 'dt_005s/ATG_PMMA_timing_01.csv',
    0.01: 'dt_001s/ATG_PMMA_timing_01.csv',
    0.001: 'dt_0001s/ATG_PMMA_timing_01.csv'
    }


timing_files = {
    100: 'dt_100s/ATG_PMMA_timing.csv',
    50:  'dt_50s/ATG_PMMA_timing.csv',
    10:  'dt_10s/ATG_PMMA_timing.csv',
    1:  'dt_1s/ATG_PMMA_timing.csv',
    0.5: 'dt_05s/ATG_PMMA_timing.csv',
    0.1: 'dt_01s/ATG_PMMA_timing.csv',
    0.05: 'dt_005s/ATG_PMMA_timing.csv',
    0.01: 'dt_001s/ATG_PMMA_timing.csv',
    0.001: 'dt_0001s/ATG_PMMA_timing.csv'
    }
...
slope2, intercept2, r_value2, p_value2, std_err2 = linregress(log_dt2[5:], log_cpu[5:])
...
plt.loglog(cpu_dt_values[4:], fit_line2[4:], '--', color='red', linewidth=4, label=f'Fit: slope={slope2:.2f}, $R^2$={r_value2**2:.3f}')
...
with open(csv_output_path, 'w', newline='') as csvfile:
    ...
    for dt in sorted(cpu_dt_values ):  # assure l’ordre croissant
        error = next((e for d, e in zip(dt_values, errorsL2) if d == dt), None)
