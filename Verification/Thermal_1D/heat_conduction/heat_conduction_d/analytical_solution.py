...
def main():
    # Case D Parameters
    L = 0.1  # m, thickness of the slab
    k = 10.0  # W/(mK), thermal conductivity
    rho = 10000  # kg/m^3, density
    c = 1000  # J/(kgK), specific heat capacity (1 kJ/kgK = 1000 J/kgK)
    h = 10  # W/(m^2K), convective heat transfer coefficient
    
    T_initial = 20.0  # °C, initial temperature of the slab
    Tg = 120.0  # °C, ambient gas temperature
    
    # Calculate thermal diffusivity (alpha) and Biot number (Bi)
    alpha = k / (rho * c)
    Bi = (h * L) / k
    
    print(f"Calculated parameters for Case D:")
    ...
    time_points = np.linspace(0,50000,200)
    ...
    output_column_names = ["Back", "8 cm", "6 cm", "4 cm", "2 cm", "Front"]
    ...
    with open(csv_filename, 'w', newline='') as csvfile:
        ...
        # Calculate and write temperatures
