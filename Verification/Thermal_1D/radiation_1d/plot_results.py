...
file_gpyro=os.path.join(SCRIPT_DIR, "radiation_1d_summary_01_0001.csv")
results_file_path = os.path.join(SCRIPT_DIR, 'radiation_1d.png')
...
style_traces = {
    "Gpyro": {"linewidth": 3},
    "Analytical": {'marker': '+',  'linestyle': 'none', 'markersize': 16, 'markevery': 4, 'markeredgewidth': 4},
}
...
plt.plot(Gpyro['t'].values, Gpyro['003_TEMPERATURE( 0.0080_ 0.0000_ 0.0000)'].values ,
         **style_traces["Gpyro"], **depth_styles["z=8"])
plt.plot(Gpyro['t'].values, Gpyro['004_TEMPERATURE( 0.0160_ 0.0000_ 0.0000)'].values ,
         **style_traces["Gpyro"], **depth_styles["z=16"])
plt.plot(Gpyro['t'].values, Gpyro['005_TEMPERATURE( 0.0400_ 0.0000_ 0.0000)'].values,
         **style_traces["Gpyro"], **depth_styles["z=40"])

# ----------- FDS -----------
plt.plot(Analytical['Time (s)'].values, Analytical['Depth 0.0 cm'].values ,
         label="Analytical", **style_traces["Analytical"], **depth_styles["z=0"])
plt.plot(Analytical['Time (s)'].values, Analytical['Depth 0.8 cm'].values,
         **style_traces["Analytical"], **depth_styles["z=8"])
plt.plot(Analytical['Time (s)'].values, Analytical['Depth 1.6 cm'].values,
         **style_traces["Analytical"], **depth_styles["z=16"])
plt.plot(Analytical['Time (s)'].values, Analytical['Depth 4.0 cm'].values ,
...
first_legend = plt.legend(handles=legend_codes, loc='lower center', bbox_to_anchor=(0.45, 0.3),  ncol=1, frameon=False)
...
error_value_1 = compute_mean_absolute_error(
    ...
    Analytical['Depth 0.0 cm'].values,
    Gpyro["t"].values,
...
error_value_2 = compute_mean_absolute_error(
    Analytical['Time (s)'].values,
    Analytical['Depth 0.8 cm'].values,
    Gpyro["t"].values,
    Gpyro['003_TEMPERATURE( 0.0080_ 0.0000_ 0.0000)'].values
    )

error_value_3 = compute_mean_absolute_error(
    Analytical['Time (s)'].values,
    Analytical['Depth 1.6 cm'].values,
    Gpyro["t"].values,
    Gpyro['004_TEMPERATURE( 0.0160_ 0.0000_ 0.0000)'].values
    )

error_value_4 = compute_mean_absolute_error(
    Analytical['Time (s)'].values,
    Analytical['Depth 4.0 cm'].values,
    Gpyro["t"].values,
    Gpyro['005_TEMPERATURE( 0.0400_ 0.0000_ 0.0000)'].values
    )
