...
!  SF%VEG_DEHYDRATION_RATE_MAX = SRF_VEG_DEHYDRATION_RATE_MAX
   ...
   LAYER_LOOP: DO IL=1,MAX_LAYERS
      IF (ADIABATIC .AND. MATL_ID(IL,1)/='null') THEN
         ...
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
      IF (GPYRO .AND. MATL_ID(IL,1)/='null') THEN
         WRITE(MESSAGE,'(A)') 'ERROR: SURF '//TRIM(SF%ID)//' is Gpyro Type and cannot have a MATL_ID. This parameter should be specified in gpyro.data File'
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF      
   ...
   !IF (SF%LAYER_DENSITY(1) > 0._EB) THEN !replaced this line with line below
   IF ((.NOT. SF%GPYRO) .AND. (SF%LAYER_DENSITY(1) > 0._EB)) THEN !NEW change: Gpyro is not consider as THERMALLY_THICK type
   ...
   ! Check for Gpyro 
   IF (SF%GPYRO) THEN
      ALLOCATE(SF%ID_GPYRO_SPECIES(MAX_SPECIES))
      ALLOCATE(SF%ID_FDS_SPECIES(MAX_SPECIES))
      SF%N_TRACKED_GPYRO_SPECIES=0D0
      DO NN=1,MAX_SPECIES
         IF (TRIM(SPEC_ID(NN))=='null') CYCLE
         DO NNN=1,N_TRACKED_SPECIES
            IF (TRIM(SPECIES_MIXTURE(NNN)%ID)==TRIM(SPEC_ID(NN))) THEN
               SF%N_TRACKED_GPYRO_SPECIES= SF%N_TRACKED_GPYRO_SPECIES +1
               SF%ID_GPYRO_SPECIES(SF%N_TRACKED_GPYRO_SPECIES)=NN
               SF%ID_FDS_SPECIES(SF%N_TRACKED_GPYRO_SPECIES)=NNN
               EXIT
            ENDIF
            IF (NNN==N_TRACKED_SPECIES) THEN
               WRITE(MESSAGE,'(A,A,A,A,A)') 'ERROR: Problem with SURF: ',TRIM(SF%ID),' SPEC ',TRIM(SPEC_ID(NN)),' not found'
      ...
      IF (SF%N_TRACKED_GPYRO_SPECIES==0) THEN
         DEALLOCATE(SF%ID_GPYRO_SPECIES)
         DEALLOCATE(SF%ID_FDS_SPECIES)
         WRITE(MESSAGE,'(A,A,A)') 'WARNING: SURF ',TRIM(SF%ID),': This is a coupled simulation with Gpyro, and no SPEC_ID was set, &
                                                      so no mass flux will be transferred from Gpyro to FDS.'
         IF (MYID==0) WRITE(LU_ERR,'(A,A,A)') TRIM(MESSAGE) 
      ENDIF
   ENDIF 
