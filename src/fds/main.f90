...
USE GPYRO_VARS, ONLY: IGPYRO_TYPE, GPYRO_BOUNDARYS_INFORMATION, GPYRO_TO_GPYRO_INTERFACE, NGPYRO_FACES_NEEDING_BCS, GPG, GP_BOUDARYS, G2GIFACE, IFACETEMP, GPM !CWL: Added this line
USE GPYRO_INIT, ONLY: ALLOCATE_GPYRO, ALLOCATE_GPYRO_BOUNDARYS_INFO, INIT_GPYRO !CWL: Added this line
USE GPYRO_IO, ONLY: READ_GPYRO, SHUTDOWN_GPYRO !CWL: Added this line
USE GPYRO_CHECK,  ONLY: CHECK_GPYRO

USE TYPES, ONLY:SURFACE_TYPE, SURFACE
USE GPYRO_VARS, ONLY: GPROP
...
CHARACTER(255), PARAMETER :: mainid='$Id: main.f90 22330 2026-04-10 18:40:12Z fds.firebot $'
CHARACTER(255), PARAMETER :: mainrev='$Revision: 92330 $'
CHARACTER(255), PARAMETER :: maindate='$Date: 2026-04-10 11:40:12 -0700 (Fri, 10 Apr 2026) $'
...
INTEGER :: ISKIP,IGPYROMESH,IRECV,IDEST,ICOUNT,LUMAP,IFROM,ITAG,ISTATUS(MPI_STATUS_SIZE),ISTART,ISTOP,ISIZE,IMESH1,IMESH2,IZ,IX,IY !CWL
INTEGER :: N_TRACKED_GPYRO_SPECIES, MAX_ID_GPYRO_SPECIES
INTEGER :: IMESH_GPYRO, JMESH_GPYRO,J
TYPE (SURFACE_TYPE), POINTER :: SF=>NULL()

TYPE (GPYRO_BOUNDARYS_INFORMATION), ALLOCATABLE, DIMENSION (:) :: MAPPING !CWL
...
DO N=0, N_SURF
   SF=> SURFACE(N)
   IF (.NOT. SF%GPYRO) CYCLE
   N_TRACKED_GPYRO_SPECIES=SF%N_TRACKED_GPYRO_SPECIES
   EXIT
ENDDO 
IF (SF%N_TRACKED_GPYRO_SPECIES>0) THEN
   MAX_ID_GPYRO_SPECIES=SF%ID_GPYRO_SPECIES(N_TRACKED_GPYRO_SPECIES)
   IF (MAX_ID_GPYRO_SPECIES> GPROP%NGSPEC) THEN
      WRITE(MESSAGE,'(1X,A,I3,A,I3,A)')  'ERROR: SURF ' // TRIM(SF%ID) // ': You are trying to associate', MAX_ID_GPYRO_SPECIES , &
               ' Gpyro gases. But only NGSPEC=', GPROP%NGSPEC ,' gases are defined in the Gpyro input file.'
...
IF (MYID .EQ. 0) NGPYRO_FACES_NEEDING_BCS(0) = SUM(NGPYRO_FACES_NEEDING_BCS(1:GPG%NUM_GPYRO_MESHES))
...
DO NM = 1, NMESHES
   ...
   DO IW=1,MESHES(NM)%N_EXTERNAL_WALL_CELLS+MESHES(NM)%N_INTERNAL_WALL_CELLS
      IF ( SURFACE(MESHES(NM)%WALL(IW)%SURF_INDEX)%THERMAL_BC_INDEX == GPYRO_BC) THEN
         MESHES(NM)%N_GPYRO_WALL_CELLS = MESHES(NM)%N_GPYRO_WALL_CELLS + 1
      ENDIF
   ENDDO
   WRITE(LU_ERR,*)
   WRITE(LU_ERR,'(1X,A,I3,A,I6)') 'Number of FDS wall cells configured for coupling with gpyro in FDS mesh', NM, ': ', MESHES(NM)%N_GPYRO_WALL_CELLS
ENDDO
WRITE(LU_ERR,'(1X,A,I7)') 'Number of Gpyro cell faces requiring boundary conditions:', NGPYRO_FACES_NEEDING_BCS(0)
WRITE(LU_ERR,'(A)')

CALL ALLOCATE_GPYRO_BOUNDARYS_INFO(GPG%NUM_GPYRO_MESHES, 0, NGPYRO_FACES_NEEDING_BCS(0))
...
DO I = 1, NGPYRO_FACES_NEEDING_BCS(IMESH_GPYRO)
      ICOUNT = 0
      IF (GP_BOUDARYS(IMESH_GPYRO)%IMESH_FDS(I) .LE. 0) CYCLE
      DO JMESH_GPYRO = 1, GPG%NUM_GPYRO_MESHES
      DO J = 1, NGPYRO_FACES_NEEDING_BCS(JMESH_GPYRO)
         IF (GP_BOUDARYS(JMESH_GPYRO)%IW_FDS   (J) .NE. GP_BOUDARYS(IMESH_GPYRO)%IW_FDS   (I)) CYCLE 
         IF (GP_BOUDARYS(JMESH_GPYRO)%IMESH_FDS(J) .NE. GP_BOUDARYS(IMESH_GPYRO)%IMESH_FDS(I)) CYCLE 
         ICOUNT = ICOUNT + 1
      ENDDO
      ENDDO
      GP_BOUDARYS(IMESH_GPYRO)%RATIO(I) = ICOUNT
ENDDO
ENDDO
...
IF (MYID .EQ. 0) THEN
   ...
   DO ICOUNT = 1, NGPYRO_FACES_NEEDING_BCS(IMESH_GPYRO)
      ...
      GP_BOUDARYS(IMESH_GPYRO)%IMESH_GPYRO(ICOUNT), &
      GP_BOUDARYS(IMESH_GPYRO)%IX_GPYRO(ICOUNT), &
      GP_BOUDARYS(IMESH_GPYRO)%IY_GPYRO(ICOUNT), &
      GP_BOUDARYS(IMESH_GPYRO)%IZ_GPYRO(ICOUNT), &
      GP_BOUDARYS(IMESH_GPYRO)%X_GPYRO(ICOUNT), &
      GP_BOUDARYS(IMESH_GPYRO)%Y_GPYRO(ICOUNT), &
      GP_BOUDARYS(IMESH_GPYRO)%Z_GPYRO(ICOUNT), &
      GP_BOUDARYS(IMESH_GPYRO)%IOR_GPYRO(ICOUNT), &
      GP_BOUDARYS(IMESH_GPYRO)%IW_FDS(ICOUNT), &
      GP_BOUDARYS(IMESH_GPYRO)%RATIO(ICOUNT), &
      GP_BOUDARYS(IMESH_GPYRO)%IMESH_FDS(ICOUNT), & 
      GP_BOUDARYS(IMESH_GPYRO)%I_FDS(ICOUNT), &
      GP_BOUDARYS(IMESH_GPYRO)%J_FDS(ICOUNT), &
      GP_BOUDARYS(IMESH_GPYRO)%K_FDS(ICOUNT), &
      GP_BOUDARYS(IMESH_GPYRO)%X_FDS(ICOUNT), &
      GP_BOUDARYS(IMESH_GPYRO)%Y_FDS(ICOUNT), &
      GP_BOUDARYS(IMESH_GPYRO)%Z_FDS(ICOUNT), &
      GP_BOUDARYS(IMESH_GPYRO)%IOR_FDS(ICOUNT), &
      GP_BOUDARYS(IMESH_GPYRO)%DIFF(ICOUNT), &
      GP_BOUDARYS(IMESH_GPYRO)%XDIFF(ICOUNT), &
      GP_BOUDARYS(IMESH_GPYRO)%YDIFF(ICOUNT), &
      GP_BOUDARYS(IMESH_GPYRO)%ZDIFF(ICOUNT)
   ENDDO
   ENDDO
   ...
   DO IMESH_GPYRO = 1, GPG%NUM_GPYRO_MESHES
   DO ICOUNT = 1, NGPYRO_FACES_NEEDING_BCS(IMESH_GPYRO)
      IF (GP_BOUDARYS(IMESH_GPYRO)%DIFF(ICOUNT) .LT. 1E6) CYCLE
      WRITE(LUMAP,3000) ICOUNT,&
      GP_BOUDARYS(IMESH_GPYRO)%IMESH_GPYRO(ICOUNT), &
      GP_BOUDARYS(IMESH_GPYRO)%IX_GPYRO(ICOUNT), &
      GP_BOUDARYS(IMESH_GPYRO)%IY_GPYRO(ICOUNT), &
...
2000 FORMAT(A)
3000 FORMAT(5(I6,','),3(F10.7,','),7(I6,','),3(F10.7,','),1(I6,','),3(E13.7,','),E13.7 )
...
!Loop over interfaces and determine T and k from various meshes
   DO ICOUNT = 1, G2GIFACE%NINTERFACES
      ...
      IF (.NOT. USE_MPI .OR. MYID .EQ. GPG%PROCESS_GPYRO(IMESH1)) THEN
         ...
         IF (ABS(G2GIFACE%IOR1(ICOUNT)) .EQ. 1) G2GIFACE%K1(ICOUNT) = GPM(IMESH1)%KX(IZ,IX,IY) 
         IF (ABS(G2GIFACE%IOR1(ICOUNT)) .EQ. 2) G2GIFACE%K1(ICOUNT) = GPM(IMESH1)%KY(IZ,IX,IY) 
         IF (ABS(G2GIFACE%IOR1(ICOUNT)) .EQ. 3) G2GIFACE%K1(ICOUNT) = GPM(IMESH1)%KZ(IZ,IX,IY)

         IF (ABS(G2GIFACE%IOR1(ICOUNT)) .EQ. 1) G2GIFACE%DLT1(ICOUNT) = GPM(IMESH1)%MESH%DX (IZ,IX,IY) 
         IF (ABS(G2GIFACE%IOR1(ICOUNT)) .EQ. 2) G2GIFACE%DLT1(ICOUNT) = GPM(IMESH1)%MESH%DY (IZ,IX,IY)  
         IF (ABS(G2GIFACE%IOR1(ICOUNT)) .EQ. 3) G2GIFACE%DLT1(ICOUNT) = GPM(IMESH1)%MESH%DZN(IZ,IX,IY)  
      ENDIF
      ...
      IF (.NOT. USE_MPI .OR. MYID .EQ. GPG%PROCESS_GPYRO(IMESH2)) THEN
         ...
         IF (ABS(G2GIFACE%IOR2(ICOUNT)) .EQ. 1) G2GIFACE%K2(ICOUNT) = GPM(IMESH2)%KX(IZ,IX,IY) 
         IF (ABS(G2GIFACE%IOR2(ICOUNT)) .EQ. 2) G2GIFACE%K2(ICOUNT) = GPM(IMESH2)%KY(IZ,IX,IY) 
         IF (ABS(G2GIFACE%IOR2(ICOUNT)) .EQ. 3) G2GIFACE%K2(ICOUNT) = GPM(IMESH2)%KZ(IZ,IX,IY) 

         IF (ABS(G2GIFACE%IOR2(ICOUNT)) .EQ. 1) G2GIFACE%DLT2(ICOUNT) = GPM(IMESH2)%MESH%DX (IZ,IX,IY)
         IF (ABS(G2GIFACE%IOR2(ICOUNT)) .EQ. 2) G2GIFACE%DLT2(ICOUNT) = GPM(IMESH2)%MESH%DY (IZ,IX,IY)
         IF (ABS(G2GIFACE%IOR2(ICOUNT)) .EQ. 3) G2GIFACE%DLT2(ICOUNT) = GPM(IMESH2)%MESH%DZN(IZ,IX,IY)
      ENDIF
...
IF (MYID==0) THEN
   ...
   MESSAGE = 'End of Gpyro simulation.' 
...
TYPE(GPYRO_BOUNDARYS_INFORMATION), INTENT(IN) :: MAPTOSEND
...
TYPE(GPYRO_BOUNDARYS_INFORMATION), INTENT(OUT) :: MAPTORECV
...
TYPE(GPYRO_BOUNDARYS_INFORMATION), INTENT(IN) :: GMAP
