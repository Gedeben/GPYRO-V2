...
USE GPYRO_VARS, ONLY: NGPYRO_FACES_NEEDING_BCS, GP_BOUDARYS, GPM, G, GPG

IMPLICIT NONE

INTEGER :: IFDSMESH,IW,ICOUNT !,LUMAP
INTEGER :: IZ,IX,IY,IFDS,JFDS,KFDS,IORFDS,IORGPYRO, IMESH_GPYRO
...
INTEGER  :: IBEST, JBEST, KBEST, IMESHBEST, IWBEST, IORBEST, NB_CELLS_NOT_ASSOCIATED
REAL(EB) :: DIFF, MINDIFF
LOGICAL  :: FIND_THE_ASSOCIATION_BETWEEN_FDS_AND_GPYRO_CELLS= .FALSE.
...
DO IFDSMESH = 1, NMESHES 
   ...
   DO IMESH_GPYRO = 1, GPG%NUM_GPYRO_MESHES
      G=>GPM(IMESH_GPYRO)
      DO ICOUNT = 1, NGPYRO_FACES_NEEDING_BCS(IMESH_GPYRO)

         FIND_THE_ASSOCIATION_BETWEEN_FDS_AND_GPYRO_CELLS=.FALSE.
         IZ         = GP_BOUDARYS(IMESH_GPYRO)%IZ_GPYRO (ICOUNT)
         IX         = GP_BOUDARYS(IMESH_GPYRO)%IX_GPYRO (ICOUNT)
         IY         = GP_BOUDARYS(IMESH_GPYRO)%IY_GPYRO (ICOUNT)
         IORGPYRO   = GP_BOUDARYS(IMESH_GPYRO)%IOR_GPYRO(ICOUNT)
         ZGPYRO     = GP_BOUDARYS(IMESH_GPYRO)%Z_GPYRO  (ICOUNT)
         XGPYRO     = GP_BOUDARYS(IMESH_GPYRO)%X_GPYRO  (ICOUNT)
         YGPYRO     = GP_BOUDARYS(IMESH_GPYRO)%Y_GPYRO  (ICOUNT)
         ...
         DO IW=1, M%N_EXTERNAL_WALL_CELLS + M%N_INTERNAL_WALL_CELLS
            ...
            XDIFF = XFDS - XGPYRO
            IF (ABS(XDIFF) .GT. 1.001D0 * DELTAX) CYCLE
            
            YDIFF = YFDS - YGPYRO
            IF (ABS(YDIFF) .GT. 1.001D0 *DELTAY) CYCLE

            ZDIFF = ZFDS - ZGPYRO
            IF (ABS(ZDIFF) .GT. 1.001D0 *DELTAZ) CYCLE
            ...
            IF (DIFF .LT. MINDIFF) THEN ! .AND. KFDS .NE. M%KBP1) THEN 
               FIND_THE_ASSOCIATION_BETWEEN_FDS_AND_GPYRO_CELLS=.TRUE.
               IMESHBEST = IFDSMESH
               IWBEST    = IW
               IBEST     = IFDS
               JBEST     = JFDS
         ...
         IF (.NOT. FIND_THE_ASSOCIATION_BETWEEN_FDS_AND_GPYRO_CELLS) THEN
            NB_CELLS_NOT_ASSOCIATED=NB_CELLS_NOT_ASSOCIATED+1
         ENDIF

         IF (FIND_THE_ASSOCIATION_BETWEEN_FDS_AND_GPYRO_CELLS) THEN
            GP_BOUDARYS(IMESH_GPYRO)%IMESH_FDS(ICOUNT) = IMESHBEST
            GP_BOUDARYS(IMESH_GPYRO)%IW_FDS   (ICOUNT) = IWBEST
            GP_BOUDARYS(IMESH_GPYRO)%I_FDS    (ICOUNT) = IBEST
            GP_BOUDARYS(IMESH_GPYRO)%J_FDS    (ICOUNT) = JBEST
            GP_BOUDARYS(IMESH_GPYRO)%K_FDS    (ICOUNT) = KBEST
            GP_BOUDARYS(IMESH_GPYRO)%IOR_FDS  (ICOUNT) = IORBEST
            GP_BOUDARYS(IMESH_GPYRO)%DIFF     (ICOUNT) = MINDIFF
            GP_BOUDARYS(IMESH_GPYRO)%XDIFF    (ICOUNT) = XDIFFBEST
            GP_BOUDARYS(IMESH_GPYRO)%YDIFF    (ICOUNT) = YDIFFBEST
            GP_BOUDARYS(IMESH_GPYRO)%ZDIFF    (ICOUNT) = ZDIFFBEST  
            GP_BOUDARYS(IMESH_GPYRO)%X_FDS    (ICOUNT) = XFDSBEST
            GP_BOUDARYS(IMESH_GPYRO)%Y_FDS    (ICOUNT) = YFDSBEST
            GP_BOUDARYS(IMESH_GPYRO)%Z_FDS    (ICOUNT) = ZFDSBEST

            ! The following forces the boundary conditions to be overwritten with default values to avoid inconsistencies
            ! and prevent using the BCs specified in the Gpyro input file.
            GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%TFIXED  =-1000 ! Negatif value mean that the temperature is not imposed
            IF (RADIATION) THEN
               GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%RERAD =.TRUE.
            ELSE
               GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%RERAD =.FALSE.
            ENDIF
         ENDIF
...
IF (NB_CELLS_NOT_ASSOCIATED .NE. 0) THEN
   WRITE(LU_ERR,'(1X, A, I6,A,I6)') "WARNING: Number of gpyro cells not associated with FDS cells =",&
   NB_CELLS_NOT_ASSOCIATED, "/", NGPYRO_FACES_NEEDING_BCS(0)
   WRITE(LU_ERR, '(1X, A, I6, A)') "The ", NB_CELLS_NOT_ASSOCIATED,  " cells are listed in the 'bad_couplings.csv' file."
   WRITE(LU_ERR, '(1X,A)')  "You might have intentionally left some gpyro cells uncoupled with FDS cells."
   WRITE(LU_ERR, '(1X,A)')  "If not, check the geometry of both gpyro and FDS."
   WRITE(LU_ERR,'(A)')
ENDIF
...
USE GPYRO_VARS, ONLY: NGPYRO_FACES_NEEDING_BCS, GP_BOUDARYS, G2GIFACE, IFACETEMP, &
                      GPG
...
IF(GPG%NUM_GPYRO_MESHES .LE. 1) RETURN

WRITE(*,*) 'ALLOCATING FOUND(:)'

ALLOCATE(FOUND(1:NGPYRO_FACES_NEEDING_BCS(0))); FOUND(:) = .FALSE.

DO I1 = 1, NGPYRO_FACES_NEEDING_BCS(0)
   IF (GP_BOUDARYS(0)%IMESH_FDS(I1) .GT. 0) FOUND(I1)=.TRUE.
ENDDO
...
DO I1 = 1, NGPYRO_FACES_NEEDING_BCS(0)
   ...
   IF (MOD(I1,1000) .EQ. 0) WRITE(*,*) 'I1: ', I1, 'OF ', NGPYRO_FACES_NEEDING_BCS(0)

   IF (FOUND(I1)) CYCLE
   
   DO I2 = 1, NGPYRO_FACES_NEEDING_BCS(0)

      IF (FOUND(I2)) CYCLE
      IF (I1 .EQ. I2) CYCLE
      IF (GP_BOUDARYS(0)%IMESH_GPYRO(I1) .EQ. GP_BOUDARYS(0)%IMESH_GPYRO(I2)) CYCLE !not need to chek for cell in the same mesh
      IF (ABS(GP_BOUDARYS(0)%IOR_GPYRO(I1)).NE. ABS(GP_BOUDARYS(0)%IOR_GPYRO(I2))) CYCLE ! only associed for cell in the same direction

      XDIFF = GP_BOUDARYS(0)%X_GPYRO(I2) - GP_BOUDARYS(0)%X_GPYRO(I1)
      IF (ABS(XDIFF) .GT. GPG%GPYRO_TO_GPYRO_TOLERANCE) CYCLE !XDIFF

      YDIFF = GP_BOUDARYS(0)%Y_GPYRO(I2) - GP_BOUDARYS(0)%Y_GPYRO(I1)
      IF (ABS(YDIFF) .GT. GPG%GPYRO_TO_GPYRO_TOLERANCE) CYCLE !YDIFF

      ZDIFF = GP_BOUDARYS(0)%Z_GPYRO(I2) - GP_BOUDARYS(0)%Z_GPYRO(I1)
      ...
      GP_BOUDARYS(0)%IMESH_FDS(I1) = -GP_BOUDARYS(0)%IMESH_GPYRO(I2)!IMESHBEST
      GP_BOUDARYS(0)%IW_FDS   (I1) = I2 !IWBEST
      GP_BOUDARYS(0)%I_FDS    (I1) = GP_BOUDARYS(0)%IX_GPYRO(I2) !IBEST
      GP_BOUDARYS(0)%J_FDS    (I1) = GP_BOUDARYS(0)%IY_GPYRO(I2) !JBEST
      GP_BOUDARYS(0)%K_FDS    (I1) = GP_BOUDARYS(0)%IZ_GPYRO(I2) !KBEST
      GP_BOUDARYS(0)%IOR_FDS  (I1) = GP_BOUDARYS(0)%IOR_GPYRO(I2) !IORBEST
      GP_BOUDARYS(0)%DIFF     (I1) = DIFF
      GP_BOUDARYS(0)%XDIFF    (I1) = XDIFF !XDIFFBEST
      GP_BOUDARYS(0)%YDIFF    (I1) = YDIFF !YDIFFBEST
      GP_BOUDARYS(0)%ZDIFF    (I1) = ZDIFF !ZDIFFBEST  
      GP_BOUDARYS(0)%X_FDS    (I1) = GP_BOUDARYS(0)%X_GPYRO(I2) !XFDSBEST
      GP_BOUDARYS(0)%Y_FDS    (I1) = GP_BOUDARYS(0)%Y_GPYRO(I2) !YFDSBEST
      GP_BOUDARYS(0)%Z_FDS    (I1) = GP_BOUDARYS(0)%Z_GPYRO(I2) !ZFDSBEST

!      GP_BOUDARYS(0)%IMESH_FDS(I2) = -GP_BOUDARYS(0)%IMESH_GPYRO(I1)!IMESHBEST
      GP_BOUDARYS(0)%IW_FDS   (I2) = I1 !IWBEST
      GP_BOUDARYS(0)%I_FDS    (I2) = GP_BOUDARYS(0)%IX_GPYRO(I1) !IBEST
      GP_BOUDARYS(0)%J_FDS    (I2) = GP_BOUDARYS(0)%IY_GPYRO(I1) !JBEST
      GP_BOUDARYS(0)%K_FDS    (I2) = GP_BOUDARYS(0)%IZ_GPYRO(I1) !KBEST
      GP_BOUDARYS(0)%IOR_FDS  (I2) = GP_BOUDARYS(0)%IOR_GPYRO(I1) !IORBEST
      GP_BOUDARYS(0)%DIFF     (I2) = DIFF
      GP_BOUDARYS(0)%XDIFF    (I2) = XDIFF !XDIFFBEST
      GP_BOUDARYS(0)%YDIFF    (I2) = YDIFF !YDIFFBEST
      GP_BOUDARYS(0)%ZDIFF    (I2) = ZDIFF !ZDIFFBEST  
      GP_BOUDARYS(0)%X_FDS    (I2) = GP_BOUDARYS(0)%X_GPYRO(I1) !XFDSBEST
      GP_BOUDARYS(0)%Y_FDS    (I2) = GP_BOUDARYS(0)%Y_GPYRO(I1) !YFDSBEST
      GP_BOUDARYS(0)%Z_FDS    (I2) = GP_BOUDARYS(0)%Z_GPYRO(I1) !ZFDSBEST
...
IF (NINTERFACES .GT. 0) THEN
   ...
   DO I1 = 1, NGPYRO_FACES_NEEDING_BCS(0)
      ...
      IF (GP_BOUDARYS(0)%IMESH_FDS(I1) .GE. 0) CYCLE
      
      ICOUNT = ICOUNT + 1

      G2GIFACE%IMATCH(ICOUNT) =  GP_BOUDARYS(0)%IW_FDS     (I1)
      G2GIFACE%IMESH1(ICOUNT) =  GP_BOUDARYS(0)%IMESH_GPYRO(I1)
      G2GIFACE%IMESH2(ICOUNT) = -GP_BOUDARYS(0)%IMESH_FDS  (I1)
      G2GIFACE%IZ1   (ICOUNT) =  GP_BOUDARYS(0)%IZ_GPYRO   (I1)
      G2GIFACE%IZ2   (ICOUNT) =  GP_BOUDARYS(0)%K_FDS      (I1)
      G2GIFACE%IX1   (ICOUNT) =  GP_BOUDARYS(0)%IX_GPYRO   (I1)
      G2GIFACE%IX2   (ICOUNT) =  GP_BOUDARYS(0)%I_FDS      (I1)
      G2GIFACE%IY1   (ICOUNT) =  GP_BOUDARYS(0)%IY_GPYRO   (I1)
      G2GIFACE%IY2   (ICOUNT) =  GP_BOUDARYS(0)%J_FDS      (I1)
      G2GIFACE%IOR1  (ICOUNT) =  GP_BOUDARYS(0)%IOR_GPYRO  (I1)
      G2GIFACE%IOR2  (ICOUNT) =  GP_BOUDARYS(0)%IOR_FDS    (I1)
...
USE MPI !CWL
USE GPYRO_VARS, ONLY: GPROP, GP_BOUDARYS, GPG, NGPYRO_FACES_NEEDING_BCS !,GPM,G2GIFACE !CWL
...
!BEGIN CWL VARS
INTEGER :: I,J,IFROM,ITAG,ISTATUS(MPI_STATUS_SIZE),IERR,ISIZE,ICOUNT,ITRANSFER,IDEST,IGPYROMESH,ISTART,ISTOP
...
!   ENDIF
   ...
   !The pyrolysis time step is the Gpyro time step.  
   !The pyrolysis time step cannot be lower than the FDS time step.  
   IF (T .LT. 0.02 .OR. (T - BC_CLOCK - GPG%DTNEXT) .GE. -EPSILON_FB) THEN ! For precision
      DT_BC    = T - BC_CLOCK
      BC_CLOCK = T
      CALL_PYROLYSIS = .TRUE.
      WALL_COUNTER = 0
...
IF (CALL_PYROLYSIS) THEN
   ...
   CALL PYROLYSIS_CWL(T,1)
   !Calculate gpyro meshes
   CALL PYROLYSIS_CWL(T,2)
   !Now transfer from Gpyro back to FDS meshes that need it.
   CALL PYROLYSIS_CWL(T,3)
...
! *****************************************************************************
SUBROUTINE PYROLYSIS_CWL(TIN,ICALL)
! *****************************************************************************

USE GPYRO_VARS, ONLY : NGPYRO_FACES_NEEDING_BCS,GP_BOUDARYS,GPM,GPG,GPROP,G,G2GIFACE,GPYRO_MESH_TYPE
USE GPYRO_FUNCS, ONLY : GET_MLR_AT_BC, GET_CPU_TIME
USE MATH_FUNCTIONS, ONLY: EVALUATE_RAMP
USE COMP_FUNCTIONS, ONLY: SHUTDOWN

REAL(EB), INTENT(IN) :: TIN
INTEGER, INTENT(IN) :: ICALL
REAL(EB) :: T,TMP_G,DTMP,HM0,YJINF,TOTAL_PRESSURE,DT, DUMMY, TSTART, TEND,ratio
REAL(EB), SAVE :: TLAST
INTEGER :: I,J,IW,IIG,JJG,KKG,ICOUNT,IMESH_FDS,N,IOR_GPYRO,IZ,IX,IY,IMESH_GPYRO,IMESH1,IMESH2,IOR,ID_GPYRO, ID_FDS
INTEGER :: ISTART, ISTOP,ICOUNT_MESH
...
TYPE (ONE_D_M_AND_E_XFER_TYPE), POINTER :: ONE_D=>NULL()
TYPE (SURFACE_TYPE), POINTER :: SF=>NULL()
TYPE (GPYRO_MESH_TYPE), POINTER :: M_GPYRO
...
SELECT CASE (ICALL)
   ...
   CASE(1) !Copy data from FDS to GP_BOUDARYS

      DO IMESH_GPYRO = 1, GPG%NUM_GPYRO_MESHES
         G=>GPM(IMESH_GPYRO)
         DO ICOUNT = 1, NGPYRO_FACES_NEEDING_BCS(IMESH_GPYRO)
            IMESH_FDS = GP_BOUDARYS(IMESH_GPYRO)%IMESH_FDS(ICOUNT)
            IF (IMESH_FDS .LE. 0) CYCLE
            IF (PROCESS(IMESH_FDS).NE. MYID) CYCLE
            IW = GP_BOUDARYS(IMESH_GPYRO)%IW_FDS(ICOUNT)
            ...
            TMP_F => ONE_D%TMP_F
            GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%T_SURFACE_OLD = TMP_F

            ! Get quantities from FDS needed by Gpyro
            TMP_G = TMP(IIG,JJG,KKG)
            IF (ASSUMED_GAS_TEMPERATURE > 0._EB) TMP_G = TMPA + EVALUATE_RAMP(T-T_BEGIN,DUMMY,I_RAMP_AGT)*(ASSUMED_GAS_TEMPERATURE-TMPA)
            DTMP = TMP_G - GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%T_SURFACE_OLD
            WALL => M%WALL
            ...
            GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%HC0   = ONE_D%HEAT_TRANS_COEF
            !GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%QENET = ONE_D%QRADIN - ONE_D%QRADOUT

            !This is a temporary trick to calculate the radiant flux from FDS without changing the code for BC in Gpyro
            IF (ONE_D%EMISSIVITY .EQ. 0D0) THEN
               GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%QE = - 5.67D-8*TMP_G**4
            ELSE
               GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%QE = (ONE_D%QRADIN /ONE_D%EMISSIVITY)- 5.67D-8*TMP_G**4
            ENDIF 
            IF (.NOT. RADIATION) GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%QE=0D0
            GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%TINF  = TMP_G
            ...
            GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%PRES  = TOTAL_PRESSURE 
            ...
            IF (GPG%SOLVE_GAS_YJ) THEN
               IF (GPG%USE_CONSTANT_HM0) THEN
                  GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%HM0 = GPG%CONSTANT_HM0
               ELSE
                  GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%HM0 = ONE_D%HEAT_TRANS_COEF / GPROP%CPG
               ENDIF
               DO N=1,N_TRACKED_SPECIES
                  GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%YJINF(N) = ZZ(IIG,JJG,KKG,N)
               ENDDO
            ENDIF
   ...
   CASE(2) !Calculate each gpyro mesh 
      ...
      DO IMESH_GPYRO = 1, GPG%NUM_GPYRO_MESHES
         ...
         CALL GPYRO_PYROLYSIS(IMESH_GPYRO,1,M_GPYRO%NCELLZ,M_GPYRO%NCELLX,M_GPYRO%NCELLY,T,FAILED_TIMESTEP)
         !WRITE(LU_ERR,'(A,F10.5, A, F10.5)') 'Calcul gpyro at T=', T, 's, with DT=', GPG%DTNEXT 
         IF (FAILED_TIMESTEP) THEN 
            WRITE(LU_ERR,'(A)')  'Gpyro: failed timestep.' 
            ...
            DO WHILE (FAILED_TIMESTEP .AND. ICOUNT .LE. 3) 
               ...
               DO I = 1, ICOUNT * 2
                  FAILED_TIMESTEP = .TRUE. 
                  CALL GPYRO_PYROLYSIS(IMESH_GPYRO,1,M_GPYRO%NCELLZ,M_GPYRO%NCELLX,M_GPYRO%NCELLY,T,FAILED_TIMESTEP)
                  IF (FAILED_TIMESTEP) EXIT
                  T = T + DT
               ENDDO
               
               IF (.NOT. FAILED_TIMESTEP) WRITE(LU_ERR,*) 'Gpyro: timestep reduction successfull. t= ', T , "with new DT=",  DT
               IF (      FAILED_TIMESTEP) WRITE(LU_ERR,*) 'Gpyro: UNSUCCESSFUL TIMESTEP REDUCTION, t= ', T, "with new DT=",  DT
         ...
         IF (FAILED_TIMESTEP) THEN
            WRITE(MESSAGE,'(A,I2,A)') "ERROR: Maximum number of time step reductions reached (=", ICOUNT-1, "): Gpyro solution has diverged"
            CALL SHUTDOWN(MESSAGE)
         ENDIF
      ...
      DO IMESH_GPYRO = 1, GPG%NUM_GPYRO_MESHES
         ...
         DO ICOUNT = 1, NGPYRO_FACES_NEEDING_BCS(IMESH_GPYRO)
            ...
            IX          = GP_BOUDARYS(IMESH_GPYRO)%IX_GPYRO (ICOUNT)
            IY          = GP_BOUDARYS(IMESH_GPYRO)%IY_GPYRO (ICOUNT)
            IOR_GPYRO   = GP_BOUDARYS(IMESH_GPYRO)%IOR_GPYRO(ICOUNT)

            GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%EMISSIVITY = CALC_EMIS(IZ,IX,IY)
            GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%T_SURFACE  = G%TPN    (IZ,IX,IY)

            IF (RADIATION) THEN
               GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%QRADOUT    = 5.67D-8 * GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%EMISSIVITY * &
                                                                  GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%T_SURFACE**4
            ELSE
               GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%QRADOUT    = 0D0
            ENDIF


            IF ( GPG%SOLVE_GAS_YJ) THEN ! ELSE is calculated outside this DO loop
               DO N=1,SF%N_TRACKED_GPYRO_SPECIES
                  ID_GPYRO=SF%ID_GPYRO_SPECIES(N)
                  HM0   = GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%HM0
                  YJINF = GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%YJINF(ID_GPYRO)
                  SELECT CASE(IOR_GPYRO)
                     CASE( 1)
                        GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%MASSFLUX(ID_GPYRO) =  G%MDOTPPDARCYE(IZ,IX,IY)*G%YJG(ID_GPYRO,IZ,IX,IY) + HM0 * (G%YJG(ID_GPYRO,IZ,IX,IY) - YJINF )
                     CASE(-1)
                        GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%MASSFLUX(ID_GPYRO) = -G%MDOTPPDARCYW(IZ,IX,IY)*G%YJG(ID_GPYRO,IZ,IX,IY) + HM0 * (G%YJG(ID_GPYRO,IZ,IX,IY) - YJINF )
                     CASE( 2)
                        GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%MASSFLUX(ID_GPYRO) =  G%MDOTPPDARCYN(IZ,IX,IY)*G%YJG(ID_GPYRO,IZ,IX,IY) + HM0 * (G%YJG(ID_GPYRO,IZ,IX,IY) - YJINF )
                     CASE(-2)
                        GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%MASSFLUX(ID_GPYRO) = -G%MDOTPPDARCYS(IZ,IX,IY)*G%YJG(ID_GPYRO,IZ,IX,IY) + HM0 * (G%YJG(ID_GPYRO,IZ,IX,IY) - YJINF )
                     CASE(-3)
                        GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%MASSFLUX(ID_GPYRO) =  G%MDOTPPDARCYB(IZ,IX,IY)*G%YJG(ID_GPYRO,IZ,IX,IY) + HM0 * (G%YJG(ID_GPYRO,IZ,IX,IY) - YJINF )
                     CASE( 3)
                        GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%MASSFLUX(ID_GPYRO) = -G%MDOTPPDARCYT(IZ,IX,IY)*G%YJG(ID_GPYRO,IZ,IX,IY) + HM0 * (G%YJG(ID_GPYRO,IZ,IX,IY) - YJINF )
                  END SELECT
               ENDDO 
            ENDIF 

            ! old way:
            GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%QCONF = GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%HC0 * &
                                 (GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%TINF - GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%T_SURFACE )

            ! new way:
            !ONE_D%QCONF = ONE_D%HEAT_TRANS_COEF * (TMP_G - 0.5_EB * (TMP_F + TMP_F_OLD) ) !from FDS pyrolysis routine
            !    GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%QCONF = GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%HC0 * (GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%TINF - 0.5D0 * (GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%T_SURFACE + GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%T_SURFACE_OLD))
            
         ENDDO

         IF ((.NOT. GPG%SOLVE_GAS_YJ) .AND. SF%N_TRACKED_GPYRO_SPECIES .GT. 0) THEN
            DO N=1,SF%N_TRACKED_GPYRO_SPECIES
               ID_GPYRO=SF%ID_GPYRO_SPECIES(N)
               CALL GET_MLR_AT_BC(ID_GPYRO,IMESH_GPYRO)
            ENDDO
         ENDIF
      ENDDO
   ...
   CASE(3) !copy Gpyro stuff back to FDS
      ...
      DO IMESH_GPYRO = 1, GPG%NUM_GPYRO_MESHES
         DO ICOUNT = 1, NGPYRO_FACES_NEEDING_BCS(IMESH_GPYRO)
            ...
            IF (IMESH_FDS .LE. 0) CYCLE
            IF (PROCESS(IMESH_FDS).NE. MYID) CYCLE
            ! Initialization to zero of the properties of FDS wall cells coupled with Gpyro
            ...
            DO N=1,SF%N_TRACKED_GPYRO_SPECIES
               ID_FDS=SF%ID_FDS_SPECIES(N)
               ONE_D%MASSFLUX(ID_FDS)        = 0D0 
               ONE_D%MASSFLUX_ACTUAL(ID_FDS) = 0D0 
            ENDDO
         ENDDO
      ENDDO
      ...
      DO IMESH_GPYRO = 1, GPG%NUM_GPYRO_MESHES
         DO ICOUNT = 1, NGPYRO_FACES_NEEDING_BCS(IMESH_GPYRO)
            ...
            ratio = REAL(GP_BOUDARYS(IMESH_GPYRO)%RATIO(ICOUNT))

            ONE_D%EMISSIVITY = ONE_D%EMISSIVITY + (GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%EMISSIVITY / ratio )
            ONE_D%TMP_F      = ONE_D%TMP_F      + (GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%T_SURFACE      / ratio)
            !ONE_D%TMP_B      = ONE_D%TMP_B      + (GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%TMP_B      /ratio )
            ONE_D%QRADOUT    = ONE_D%QRADOUT    + (GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%QRADOUT    / ratio )
            ONE_D%QCONF = ONE_D%QCONF + (GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%QCONF / ratio)
            ...
            DO N=1,SF%N_TRACKED_GPYRO_SPECIES
               ...
               ID_GPYRO=SF%ID_GPYRO_SPECIES(N)
               IF (GPG%SOLVE_GAS_YJ) THEN
                  ONE_D%MASSFLUX(ID_FDS)        = ONE_D%MASSFLUX(ID_FDS)        + (GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%MASSFLUX(ID_GPYRO) / ratio )
                  ONE_D%MASSFLUX_ACTUAL(ID_FDS) = ONE_D%MASSFLUX_ACTUAL(ID_FDS) + (GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%MASSFLUX(ID_GPYRO) / ratio )
               ELSE
                  ONE_D%MASSFLUX(ID_FDS)        = ONE_D%MASSFLUX(ID_FDS)        + (GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%MLR(ID_GPYRO) / WC%AW)
                  ONE_D%MASSFLUX_ACTUAL(ID_FDS) = ONE_D%MASSFLUX_ACTUAL(ID_FDS) + (GP_BOUDARYS(IMESH_GPYRO)%GPBC(ICOUNT)%MLR(ID_GPYRO) / WC%AW)
               ENDIF
            ENDDO
