...
DO WHILE ((CI%ITER .LE. GPG%NTDMA_ITERATIONS) .AND. (.NOT.CI%CONVERGED_ALL) .AND. (.NOT. GPG%NAN))
   ...
   IF (UPDATE_COEFFS .AND. (GPG%SOLVE_PRESSURE .OR. GPG%SOLVE_GAS_YJ .OR. GPG%SOLVE_GAS_ENERGY)) THEN 
      ...
      CALL CALCULATE_GAS_DENSITY(IMESH)
   ...
   IF (GPG%SOLVE_GAS_YJ  .AND. (.NOT. CI%CONVERGED_YJG(0))) &
   ...
   IF (GPG%SOLVE_GAS_ENERGY) THEN
      ...
      IF (UPDATE_COEFFS .AND. (.NOT. GPG%THERMAL_EQUILIBRIUM) .AND. GPG%HCV .LT. 0D0) THEN
         CALL CALC_HCV(IMESH,NCELLZ,NCELLX,NCELLY)
      ENDIF
      CALL GAS_ENERGY_SOURCE_TERMS(IMESH)
      !!!! THERE IS NO GAS ENTHALPY SOLVER!!!!!!!
   ENDIF
   CALL GET_CPU_TIME(TMID2);  GPG%TUSED(30) = GPG%TUSED(30) + TMID2 - TMID1
   !===============================================================!
ENDDO !ITER
...
G%TPN(1,1,1) = G%TP (1,1,1) + GPG%BETA(ICASE) * DTIME / 60D0
...
IF (G%MASS_N(1,1,1) .LE. EPSILON_EB) G%CONSUMED(1,1,1) = .TRUE. 
...
IF (.NOT. CONVERGED) THEN
   ...
   ELSE
      ...
      M%DZN   (1,1,1)   = M%DZ(1,1,1)
      G%MASS_N(1,1,1)   = G%RP(1,1,1)*M%DZ(1,1,1)
...
IF(IGPYRO_TYPE .EQ. 1 ) CALL DUMP_GPYRO(IMESH,ICASE,TI)
...
IF (ASSOCIATED(M%DZ, G%STORAGE%DZ_W1)) THEN
   M%DZ => G%STORAGE%DZ_W2
   M%DZN => G%STORAGE%DZ_W1
   !G%STORAGE%DZ_W1 = -HUGE(0._EB)
ELSE IF (ASSOCIATED(M%DZ, G%STORAGE%DZ_W2)) THEN
   M%DZ => G%STORAGE%DZ_W1
   M%DZN => G%STORAGE%DZ_W2
   !G%STORAGE%DZ_W2 = -HUGE(0._EB)
END IF

IF (ASSOCIATED(G%MASS, G%STORAGE%MASS_W1)) THEN
   G%MASS => G%STORAGE%MASS_W2
   G%MASS_N => G%STORAGE%MASS_W1
   !G%STORAGE%MASS_W1 = -HUGE(0._EB)
ELSE IF (ASSOCIATED(G%MASS, G%STORAGE%MASS_W2)) THEN
   G%MASS => G%STORAGE%MASS_W1
   G%MASS_N => G%STORAGE%MASS_W2
   !G%STORAGE%MASS_W2 = -HUGE(0._EB)
END IF
...
IF (ASSOCIATED(G%MASS_I, G%STORAGE%MASS_I_W1)) THEN
   G%MASS_I => G%STORAGE%MASS_I_W2
   G%MASS_IN => G%STORAGE%MASS_I_W1
   !G%STORAGE%MASS_I_W1 = HUGE(0._EB)
ELSE IF (ASSOCIATED(G%MASS_I, G%STORAGE%MASS_I_W2)) THEN
   G%MASS_I => G%STORAGE%MASS_I_W1
   G%MASS_IN => G%STORAGE%MASS_I_W2
   !G%STORAGE%MASS_I_W2 = HUGE(0._EB)
END IF

IF (ASSOCIATED(G%TIME_INTEGRATED_MASS_PRODUCED, G%STORAGE%TIME_INTEGRATED_MASS_PRODUCED_W1)) THEN
   G%TIME_INTEGRATED_MASS_PRODUCED => G%STORAGE%TIME_INTEGRATED_MASS_PRODUCED_W2
   G%TIME_INTEGRATED_MASS_PRODUCEDN => G%STORAGE%TIME_INTEGRATED_MASS_PRODUCED_W1
   !G%STORAGE%TIME_INTEGRATED_MASS_PRODUCED_W1 = HUGE(0._EB)
ELSE IF (ASSOCIATED(G%TIME_INTEGRATED_MASS_PRODUCED, G%STORAGE%TIME_INTEGRATED_MASS_PRODUCED_W2)) THEN
   ...
   !G%STORAGE%TIME_INTEGRATED_MASS_PRODUCED_W2 = HUGE(0._EB)
END IF
...
!!$OMP SHARED(NGPYRO_FACES_NEEDING_BCS, IMESH, BOUNDARYS)
DO ICOUNT = 1, NGPYRO_FACES_NEEDING_BCS(IMESH)
   ...
   BOUNDARYS%GPBC(ICOUNT)%T_SURFACE_OLD = BOUNDARYS%GPBC(ICOUNT)%T_SURFACE
ENDDO
!!$OMP END PARALLEL DO

IF (NCELLX .EQ. 1 .AND. NCELLY .EQ. 1) THEN
   IF (G%THICKNESS(1,1) .GT. 0D0) THEN
      G%THICKNESS(1,1) = SUM(M%DZ(:,1,1))
   ENDIF
ENDIF
...
DO IZ = 1, NCELLZ
   ...
   IF (G%MASS(IZ,IX,IY)/G%INITIAL_MASS(0,IZ,IX,IY) .LE. GPG%EPS) THEN
...
DO IZ = 1, NCELLZ
   ...
   M%DZ  (IZ,IX,IY) = M%DZN (IZ,IX,IY)
   G%MASS (IZ,IX,IY) = G%MASS_N(IZ,IX,IY)

   DO ISSPEC = 1, SPROP%NSSPEC
      G%YI        (ISSPEC,IZ,IX,IY)   = G%YIN        (ISSPEC,IZ,IX,IY)
      G%XI        (ISSPEC,IZ,IX,IY)   = G%XIN        (ISSPEC,IZ,IX,IY)
      G%MASS_I    (ISSPEC,IZ,IX,IY)   = G%MASS_IN    (ISSPEC,IZ,IX,IY)
      G%TIME_INTEGRATED_MASS_PRODUCED(ISSPEC,IZ,IX,IY)   = G%TIME_INTEGRATED_MASS_PRODUCEDN(ISSPEC,IZ,IX,IY)
   ENDDO
...
DO IZ = 1, NCELLZ
   IF(G%CONSUMED(IZ,IX,IY) .OR. M%IMASK(IZ,IX,IY) .OR. (.NOT. G%IS_REACTING(0,IZ,IX,IY))) CYCLE
   IF (G%MASS_N(IZ,IX,IY)/G%INITIAL_MASS(0,IZ,IX,IY) .LE. GPG%EPS) THEN
      G%CONSUMED(IZ,IX,IY) = .TRUE.
   ENDIF
ENDDO
ENDDO
...
M%DZN   (:,IXP,IYP)   = M%DZ(:,IXP,IYP)
M%DV    (:,IXP,IYP)   = M%DZN   (:,IXP,IYP)*M%DXDY(:,IXP,IYP)
G%MASS_N(:,IXP,IYP)   = G%RP(:,IXP,IYP)*M%DV(:,IXP,IYP)

IF (G%THICKNESS(IXP,IYP) .GT. 0D0) THEN
   G%THICKNESS(IXP,IYP) = SUM(M%DZ(:,IXP,IYP))
ENDIF
...
DO ICOUNT = 1, NGPYRO_FACES_NEEDING_BCS(IMESH)
   ...
   IY  = BOUNDARYS%IY_GPYRO (ICOUNT)
   IOR = BOUNDARYS%IOR_GPYRO (ICOUNT)
   BOUNDARYS%GPBC(ICOUNT)%T_SURFACE = BOUNDARYS%GPBC(ICOUNT)%T_SURFACE_OLD
END DO
...
!$OMP DEFAULT(SHARED)
DO IZ = 1, M%NCELLZ
   IF(G%CONSUMED(IZ,IXP,IYP) .OR. M%IMASK(IZ,IXP,IYP) .OR. (.NOT. G%IS_REACTING(0,IZ,IXP,IYP))) CYCLE
   IF (G%MASS_N(IZ,IXP,IYP)/G%INITIAL_MASS(0,IZ,IXP,IYP) .LE. GPG%EPS) THEN
...
! In this case we should solve the updated density
IF (GPG%IS_DENSITY_DEPENDENT_ON_TEMPERATURE) CI%CONVERGED_YIS(:) =.FALSE.
