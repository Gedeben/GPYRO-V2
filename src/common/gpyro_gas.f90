...
!$OMP SHARED(AP0,AP0N,DTIME,G,M,GPROP,NCELLX,NCELLY,NCELLZ) COLLAPSE(3)
DO IY = 1, NCELLY
DO IX = 1, NCELLX
DO IZ = 1, NCELLZ
   ! M in g.mol-1
   AP0 (IZ,IX,IY) = (G%M (IZ,IX,IY)/(1000*R)) * G%POROSS (IZ,IX,IY) * M%DZ (IZ,IX,IY) * M%DXDY(IZ,IX,IY) / (TG (IZ,IX,IY) * DTIME)
   AP0N(IZ,IX,IY) = (G%MN(IZ,IX,IY)/(1000*R)) * G%POROSSN(IZ,IX,IY) * M%DZN(IZ,IX,IY) * M%DXDY(IZ,IX,IY) / (TGN(IZ,IX,IY) * DTIME)
ENDDO
ENDDO
ENDDO
!$OMP END PARALLEL DO
...
DO IZ = 1, NCELLZ
   IF (M%IMASK(IZ,IX,IY)) THEN
      ...
      CYCLE
   ENDIF
   AB(IZ,IX,IY) = G%PERMONUB(IZ,IX,IY) * M%GZB(IZ,IX,IY)
   AT(IZ,IX,IY) = G%PERMONUT(IZ,IX,IY) * M%GZT(IZ,IX,IY)
   AE(IZ,IX,IY) = G%PERMONUE(IZ,IX,IY) * M%GXE(IZ,IX,IY)
   AW(IZ,IX,IY) = G%PERMONUW(IZ,IX,IY) * M%GXW(IZ,IX,IY)
   AN(IZ,IX,IY) = G%PERMONUN(IZ,IX,IY) * M%GYN(IZ,IX,IY)
   AS(IZ,IX,IY) = G%PERMONUS(IZ,IX,IY) * M%GYS(IZ,IX,IY)
ENDDO
ENDDO
ENDDO
!$OMP END PARALLEL DO 
...
DO IZ = 1, NCELLZ
   ...
   B (IZ,IX,IY) = AP0(IZ,IX,IY) * G%P(IZ,IX,IY) + &
                  ...
                  G%OMEGASFG(IZ,IX,IY) * M%DZN(IZ,IX,IY) * M%DX (IZ,IX,IY) * M%DY (IZ,IX,IY)
...
DO ICOUNT = 1, NGPYRO_FACES_NEEDING_BCS(IMESH)
   ...
   IF (ABS(IOR)==3)  DS_OR = M%DXDY(IZ,IX,IY)
   IF (IOR ==  1)  DS_OR = M%SXE(IZ,IX,IY)
   IF (IOR == -1)  DS_OR = M%SXW(IZ,IX,IY)
   IF (IOR ==  2)  DS_OR = M%SYN(IZ,IX,IY)
   IF (IOR == -2)  DS_OR = M%SYS(IZ,IX,IY)

   IF (BOUNDARYS%GPBC(ICOUNT)%PRES .LT. 0D0) THEN
      B(IZ,IX,IY) = B(IZ,IX,IY) + BOUNDARYS%GPBC(ICOUNT)%MFLUX*DS_OR
   ELSE 
      CALL SET_BC_FIXED_VALUE(AP(IZ,IX,IY),AB(IZ,IX,IY),AT(IZ,IX,IY),AE(IZ,IX,IY),&
                              AW(IZ,IX,IY),AN(IZ,IX,IY),AS(IZ,IX,IY),B(IZ,IX,IY),&
                              BOUNDARYS%GPBC(ICOUNT)%PRES)
   ENDIF
ENDDO
...
ELSE
   ...
   DO IZ = 1, NCELLZ
      ...
      IF (G%RESIDUAL_P(IZ,IX,IY) .GT. GPG%PTOL*ALPHA_P) THEN
         ALL_CELLS_CONVERGED = .FALSE.
      ENDIF
      IF (.NOT. IEEE_IS_FINITE(G%PN(IZ,IX,IY)) .OR. G%PN(IZ,IX,IY).GT.1D15 .OR. G%PN(IZ,IX,IY).LT. -1D15) THEN
         GPG%NAN = .TRUE.
         ALL_CELLS_CONVERGED = .FALSE.
      ENDIF
   ENDDO
...
INTEGER, INTENT(IN) :: IMESH
INTEGER :: IZ_IN,IZ,IX,IY,IOR,ICOUNT
INTEGER :: IZ_OUT
INTEGER :: NCELLZ,NCELLX,NCELLY
TYPE (GPYRO_MESH_TYPE), POINTER :: M
TYPE (GPYRO_BOUNDARYS_INFORMATION), POINTER :: BOUNDARYS
...
!$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(IX,IY,IZ,IZ_IN,IOR,ICOUNT) &
!$OMP DEFAULT(SHARED)
DO ICOUNT = 1, NGPYRO_FACES_NEEDING_BCS(IMESH)
   IOR = BOUNDARYS%IOR_GPYRO(ICOUNT)
   IF (IOR .NE. -3) CYCLE ! start at the cell at the bottom surface
   IZ_IN = BOUNDARYS%IZ_GPYRO (ICOUNT)
   IX    = BOUNDARYS%IX_GPYRO (ICOUNT)
   IY    = BOUNDARYS%IY_GPYRO (ICOUNT)

   ! The geometry is upside down, meaning that the z axis goes from the upper surface to the bottom surface
   ! So with instantaneous release the mass flux goes against the z axis direction, hence the negative sign

   ! Account for mass flux due to bottom boundary condition
    IF (BOUNDARYS%GPBC(ICOUNT)%MFLUX .GT. 0D0) THEN
      G%MDOTPPZB(IZ_IN,IX,IY) = -BOUNDARYS%GPBC(ICOUNT)%MFLUX
   ENDIF

   DO IZ = IZ_IN-1, 1, -1
      IZ_OUT = IZ
      IF (M%IMASK(IZ,IX,IY)) THEN
         IZ_OUT = IZ +1 ! We are outside the solid, therefore IZ_OUT is the last cell
         EXIT 
      ENDIF
      G%MDOTPPZB(IZ  ,IX,IY) = G%MDOTPPZB(IZ+1,IX,IY) - G%OMEGASFG(IZ+1,IX,IY) * M%DZ(IZ,IX,IY)
      G%MDOTPPZT(IZ+1,IX,IY) = G%MDOTPPZB(IZ  ,IX,IY)
   ENDDO
   G%MDOTPPZT(IZ_OUT,IX,IY)= G%MDOTPPZT(IZ_OUT+1,IX,IY) - G%OMEGASFG(IZ_OUT,IX,IY) * M%DZ(IZ_OUT,IX,IY)

   DO IZ = IZ_IN, IZ_OUT, -1
      G%MDOTPPZ(0,IZ,IX,IY)= 0.5 * (G%MDOTPPZT(IZ,IX,IY)+G%MDOTPPZB(IZ,IX,IY))
   ENDDO
ENDDO
!$OMP END PARALLEL DO
...
SELECT CASE(DIRECTION) 
   ...
   CASE ('Z')
      !$OMP PARALLEL DO PRIVATE(IX,IY,IZ,DPDZMRG)&
      !$OMP SHARED(G,M,NCELLX,NCELLY,NCELLZ) SCHEDULE(STATIC) COLLAPSE(2)
      DO IY = 1, NCELLY
      DO IX = 1, NCELLX
         DO IZ = 1, NCELLZ-1 !Calculate z-direction mass flux (bottom)
            DPDZMRG = (G%PN(IZ+1,IX,IY) - G%PN(IZ,IX,IY)) / M%DZB(IZ,IX,IY) !- G%RGNB(IZ,IX,IY)*G%GZ  
   ...
   CASE('X')
      !$OMP PARALLEL DO PRIVATE(IX,IY,IZ,DPDXMRG) SHARED(G,M,NCELLX,NCELLY,NCELLZ) SCHEDULE(STATIC) COLLAPSE(2)
      DO IY = 1, NCELLY
      DO IZ = 1, NCELLZ
         DO IX = 1, NCELLX-1 !Calculate x-direction mass flux (east)
            DPDXMRG = (G%PN(IZ,IX+1,IY) - G%PN(IZ,IX,IY)) / M%DXE(IZ,IX,IY) !- G%RGNE(IZ,IX,IY)*G%GX  
   ...
   CASE('Y')
      !$OMP PARALLEL DO PRIVATE(IX,IY,IZ,DPDYMRG) SHARED(G,M,NCELLX,NCELLY,NCELLZ) SCHEDULE(STATIC) COLLAPSE(2)
      DO IX = 1, NCELLX
      DO IZ = 1, NCELLZ
         DO IY = 1, NCELLY-1 !Calculate y-direction mass flux (north)
            DPDYMRG = (G%PN(IZ,IX,IY+1) - G%PN(IZ,IX,IY)) / M%DYN(IZ,IX,IY) !- G%RGNN(IZ,IX,IY)*G%GY  
...
IF (GPG%SOLVE_GAS_YJ) THEN
   ...
   IF (G%CONV_INFO%ITER .EQ. 1) THEN
      !The Gas mass fraction used is that at the last iteration
...
IF (G%CONV_INFO%ITER .EQ. 1) THEN
   !$OMP PARALLEL DO PRIVATE(IX,IY,IZ,IGSPEC) SCHEDULE(STATIC) COLLAPSE(3)&
   !$OMP SHARED(G,M,AP,AB,AT,AE,AW,AN,AS,AP0,AP0YJG,RDTIME,NGSPEC,NCELLX,NCELLY,NCELLZ) 
   DO IY = 1, NCELLY
   DO IX = 1, NCELLX
   DO IZ = 1, NCELLZ
      IF (M%IMASK(IZ,IX,IY)) THEN
         ...
         AN(IZ,IX,IY) = 0D0 ; AS(IZ,IX,IY) = 0D0 
      ELSE
         AP0(IZ,IX,IY) = G%RG(IZ,IX,IY)*M%DV(IZ,IX,IY)*G%POROSS(IZ,IX,IY) * RDTIME
         DO IGSPEC = 1, NGSPEC
...
IF (UPDATE_COEFFS) THEN
   !$OMP PARALLEL DO PRIVATE(IX,IY,IZ) DEFAULT(SHARED) SCHEDULE(STATIC) COLLAPSE(3)
   DO IY = 1, NCELLY
   DO IX = 1, NCELLX
   DO IZ = 1, NCELLZ
      IF (M%IMASK(IZ,IX,IY)) CYCLE
      ...
      IF (NCELLZ .GT. 1 ) THEN
         DT(IZ,IX,IY) = G%PSIRGDT(IZ,IX,IY) / M%DZT(IZ,IX,IY)
         DB(IZ,IX,IY) = G%PSIRGDB(IZ,IX,IY) / M%DZB(IZ,IX,IY)
      ENDIF
      IF (NCELLX .GT. 1 ) THEN
         DW(IZ,IX,IY) = G%PSIRGDW(IZ,IX,IY) / M%DXW(IZ,IX,IY)
         DE(IZ,IX,IY) = G%PSIRGDE(IZ,IX,IY) / M%DXE(IZ,IX,IY)
      ENDIF
      IF (NCELLY .GT. 1) THEN
         DS(IZ,IX,IY) = G%PSIRGDS(IZ,IX,IY) / M%DYS(IZ,IX,IY)
         DN(IZ,IX,IY) = G%PSIRGDN(IZ,IX,IY) / M%DYN(IZ,IX,IY)
      ENDIF
   ENDDO
   ENDDO
   ENDDO
   ...
   DO ICOUNT = 1, NGPYRO_FACES_NEEDING_BCS(IMESH)
      ...
      IY  = BOUNDARYS%IY_GPYRO (ICOUNT)
      IOR = BOUNDARYS%IOR_GPYRO(ICOUNT)
      HM  = BOUNDARYS%GPBC(ICOUNT)%HM0

      SELECT CASE(IOR)
         CASE( 3); DT(IZ,IX,IY) = HM
         CASE(-3); DB(IZ,IX,IY) = HM
         CASE( 1); DE(IZ,IX,IY) = HM
         CASE(-1); DW(IZ,IX,IY) = HM
         CASE( 2); DN(IZ,IX,IY) = HM
         CASE(-2); DS(IZ,IX,IY) = HM
      END SELECT
   ENDDO
   ...
   DO IZ = 1, NCELLZ
      ...
      IF (NCELLZ .GT. 1 ) THEN
         ...
         AT(IZ,IX,IY) = (DT(IZ,IX,IY) * BIGAT(IZ,IX,IY) + MAX ( MDOTPPT(IZ,IX,IY), 0D0)) * M%DXDY(IZ,IX,IY)
         AB(IZ,IX,IY) = (DB(IZ,IX,IY) * BIGAB(IZ,IX,IY) + MAX (-MDOTPPB(IZ,IX,IY), 0D0)) * M%DXDY(IZ,IX,IY)
      ENDIF
      ...
      IF (NCELLX .GT. 1 ) THEN
         ...
         AW(IZ,IX,IY) = (DW(IZ,IX,IY) * BIGAW(IZ,IX,IY) + MAX ( MDOTPPW(IZ,IX,IY), 0D0) ) * M%SXW(IZ,IX,IY)
         AE(IZ,IX,IY) = (DE(IZ,IX,IY) * BIGAE(IZ,IX,IY) + MAX (-MDOTPPE(IZ,IX,IY), 0D0) ) * M%SXE(IZ,IX,IY)
      ENDIF
      ...
      IF (NCELLY .GT. 1) THEN
         ...
         AS(IZ,IX,IY) = (DS(IZ,IX,IY) * BIGAS(IZ,IX,IY) + MAX ( MDOTPPS(IZ,IX,IY), 0D0) ) * M%SYS(IZ,IX,IY)
         AN(IZ,IX,IY) = (DN(IZ,IX,IY) * BIGAN(IZ,IX,IY) + MAX (-MDOTPPN(IZ,IX,IY), 0D0) ) * M%SYN(IZ,IX,IY)
      ENDIF
   ENDDO
   ENDDO
   ENDDO
   ...
   DO ICOUNT = 1, NGPYRO_FACES_NEEDING_BCS(IMESH)
      ...
      SELECT CASE(IOR)
         CASE( 3); ATIZT(IZ,IX,IY) = AT(IZ,IX,IY) ; AT(IZ,IX,IY) = 0D0
         CASE(-3); ABIZB(IZ,IX,IY) = AB(IZ,IX,IY) ; AB(IZ,IX,IY) = 0D0
         CASE( 1); AEIXE(IZ,IX,IY) = AE(IZ,IX,IY) ; AE(IZ,IX,IY) = 0D0
         CASE(-1); AWIXW(IZ,IX,IY) = AW(IZ,IX,IY) ; AW(IZ,IX,IY) = 0D0
         CASE( 2); ANIYN(IZ,IX,IY) = AN(IZ,IX,IY) ; AN(IZ,IX,IY) = 0D0
         CASE(-2); ASIYS(IZ,IX,IY) = AS(IZ,IX,IY) ; AS(IZ,IX,IY) = 0D0
      END SELECT

   ENDDO  

   !$OMP PARALLEL DO PRIVATE(IX,IY,IZ,IGSPEC,apg) DEFAULT(SHARED) SCHEDULE(STATIC) COLLAPSE(3)
   DO IY = 1, NCELLY
   DO IX = 1, NCELLX
   DO IZ = 1, NCELLZ
      IF (M%IMASK(IZ,IX,IY)) CYCLE
      apg = AP0(IZ,IX,IY) + G%OMEGASFG(IZ,IX,IY)*M%DV(IZ,IX,IY)
      IF (NCELLZ .GT. 1) apg = apg + AT(IZ,IX,IY) + AB(IZ,IX,IY)
      IF (NCELLX .GT. 1) apg = apg + AE(IZ,IX,IY) + AW(IZ,IX,IY) 
      IF (NCELLY .GT. 1) apg = apg + AN(IZ,IX,IY) + AS(IZ,IX,IY)
      AP1(IZ,IX,IY) = apg
      DO IGSPEC = 1, NGSPEC
         GOMEGA3DXDYDZ(IGSPEC,IZ,IX,IY) = G%GOMEGA(3,IGSPEC,IZ,IX,IY) * M%DV(IZ,IX,IY)
      ENDDO
   ENDDO
   ENDDO
   ENDDO
...
DO IGSPEC = 1, GPROP%NGSPEC
   ...
   !IF (ITER .GT. 1 .AND. MAXVAL(G%RESIDUAL_YJG(IGSPEC,:,:,:)*REAL(M%IMASK(:,:,:)))*RALPHA_YJG .LT. 1D-30) CYCLE
   ...
   DO ICOUNT = 1, NGPYRO_FACES_NEEDING_BCS(IMESH)
      ...
      IY  = BOUNDARYS%IY_GPYRO (ICOUNT)
      IOR = BOUNDARYS%IOR_GPYRO (ICOUNT)
      dyg  = BOUNDARYS%GPBC(ICOUNT)%YJINF(IGSPEC)-G%YJGN(IGSPEC,IZ,IX,IY)
      SELECT CASE(IOR)
         CASE( 3); BBC(IZ,IX,IY) = BBC(IZ,IX,IY) + ATIZT(IZ,IX,IY)*dyg
         CASE(-3); BBC(IZ,IX,IY) = BBC(IZ,IX,IY) + ABIZB(IZ,IX,IY)*dyg
         CASE( 1); BBC(IZ,IX,IY) = BBC(IZ,IX,IY) + AEIXE(IZ,IX,IY)*dyg
         CASE(-1); BBC(IZ,IX,IY) = BBC(IZ,IX,IY) + AWIXW(IZ,IX,IY)*dyg
         CASE( 2); BBC(IZ,IX,IY) = BBC(IZ,IX,IY) + ANIYN(IZ,IX,IY)*dyg
         CASE(-2); BBC(IZ,IX,IY) = BBC(IZ,IX,IY) + ASIYS(IZ,IX,IY)*dyg
      END SELECT
   ENDDO
   ...
   IF (SWEEPX) THEN
      ...
      DO IZ = 1, NCELLZ
         ...
         IF ( IZ .NE. 1     ) BZ (IZ,IX,IY) = BZ (IZ,IX,IY) + AT(IZ,IX,IY) * G%YJGN(IGSPEC,IZ-1,IX,IY)
         IF ( IZ .NE. NCELLZ) BZ (IZ,IX,IY) = BZ (IZ,IX,IY) + AB(IZ,IX,IY) * G%YJGN(IGSPEC,IZ+1,IX,IY)
         IF ( IY .NE. 1     ) BY (IZ,IX,IY) = BY (IZ,IX,IY) + AS(IZ,IX,IY) * G%YJGN(IGSPEC,IZ,IX,IY-1)
         IF ( IY .NE. NCELLY) BY (IZ,IX,IY) = BY (IZ,IX,IY) + AN(IZ,IX,IY) * G%YJGN(IGSPEC,IZ,IX,IY+1)
      ENDDO
      ENDDO
      ENDDO
      !$OMP END PARALLEL DO
   ...
   IF (SWEEPY) THEN
      ...
      DO IZ = 1, NCELLZ
         ...
         IF ( IZ .NE. 1     ) BZ (IZ,IX,IY) = BZ (IZ,IX,IY) + AT(IZ,IX,IY) * G%YJGN(IGSPEC,IZ-1,IX,IY)
         IF ( IZ .NE. NCELLZ) BZ (IZ,IX,IY) = BZ (IZ,IX,IY) + AB(IZ,IX,IY) * G%YJGN(IGSPEC,IZ+1,IX,IY)
         IF ( IX .NE. 1     ) BX (IZ,IX,IY) = BX (IZ,IX,IY) + AW(IZ,IX,IY) * G%YJGN(IGSPEC,IZ,IX-1,IY)
         IF ( IX .NE. NCELLX) BX (IZ,IX,IY) = BX (IZ,IX,IY) + AE(IZ,IX,IY) * G%YJGN(IGSPEC,IZ,IX+1,IY)
      ENDDO
      ENDDO
      ENDDO
      !$OMP END PARALLEL DO
...
ELSE
   ...
   DO IZ = 1, NCELLZ
      ...
      DO IGSPEC = 1, GPROP%NGSPEC
         ...
         IF (.NOT. IEEE_IS_FINITE(G%YJGN(IGSPEC,IZ,IX,IY))) THEN
...
!$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(IX,IY,IZ) &
!$OMP SHARED(G,GPROP,GPG,NCELLX,NCELLY,NCELLZ,NGSPEC) COLLAPSE(3)
DO IY = 1, NCELLY
DO IX = 1, NCELLX
DO IZ = 1, NCELLZ
   IF (GPG%SOLVE_PRESSURE) THEN
      G%RGN(IZ,IX,IY) = RHOGOFT(G%PN(IZ,IX,IY), G%MN(IZ,IX,IY), G%TP(IZ,IX,IY))
   ELSE
      G%RGN(IZ,IX,IY) = RHOGOFT(GPG%P0, G%MN(IZ,IX,IY), G%TP(IZ,IX,IY))
   ENDIF
ENDDO
ENDDO
ENDDO
   ...
   CASE('PERMONU')
      ...
      IF (NCELLZ .GT. 1 .AND. NCELLX .LE. 1) THEN
         ...
         DO IZ = 1, NCELLZ
            ...
            IF(M%IMASK(IZ,IX,IY)) GAMPZ(IZ,IX,IY) = GAMPZ(IZ,IX,IY) * 1D6
         ENDDO
         !$OMP END PARALLEL DO
         GAMT => G%PERMONUT
         GAMB => G%PERMONUB
