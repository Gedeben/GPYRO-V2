...
! *****************************************************************************      
SUBROUTINE GET_EXIT_GAS_CELL(IMESH)
! *****************************************************************************      
...
! *****************************************************************************
END SUBROUTINE GET_EXIT_GAS_CELL
! *****************************************************************************
...
! *****************************************************************************
SUBROUTINE CALCULATE_INTERFACE_QUANTITIES(IMESH,GAMPZ,GAMPX,GAMPY, &
                                          GAMT,GAMB,GAME,GAMW,GAMN,GAMS)
! *****************************************************************************

INTEGER, INTENT(IN) :: IMESH
!"Gamma", from Patankar nomenclature 
REAL(EB), INTENT(IN), POINTER, DIMENSION(:,:,:) :: GAMPZ, GAMPX, GAMPY
REAL(EB), INTENT(IN), POINTER, DIMENSION(:,:,:) :: GAMT,GAMB,GAME,GAMW,GAMN,GAMS
...
IF (NCELLZ .GT. 1 .AND. NCELLX .EQ. 1) THEN
   IX = 1; IY = 1
   !$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(IZ) SHARED(GAMPZ,NCELLZ)
   DO IZ = 1, NCELLZ
...
DO ICOUNT = 1, NGPYRO_FACES_NEEDING_BCS(IMESH)
   ...
   SELECT CASE(IOR)
      ...
      CASE(-3) ; GAMPZ(IZ+1,IX,IY) = GAMPZ(IZ,IX,IY)
      CASE( 1) ; GAMPX(IZ,IX+1,IY) = GAMPX(IZ,IX,IY)
      CASE(-1) ; GAMPX(IZ,IX-1,IY) = GAMPX(IZ,IX,IY)
      CASE( 2) ; GAMPY(IZ,IX,IY+1) = GAMPY(IZ,IX,IY)
      CASE(-2) ; GAMPY(IZ,IX,IY-1) = GAMPY(IZ,IX,IY)
   END SELECT
...
! *****************************************************************************
END FUNCTION MY_DERF
! *****************************************************************************      

! *****************************************************************************      
REAL(EB) FUNCTION MY_DERFC(X) 
! *****************************************************************************      
...
DIR = TRIM(GPG%SOLVER_MAIN_DIRECTIONS)
...
CASE ('z','Z')
   SWEEPZ = .TRUE.
CASE ('xy','XY')
   IF (MOD(G%NTIMESTEPS,2) .EQ. 0) SWEEPX = .TRUE. 
   IF (MOD(G%NTIMESTEPS,2) .EQ. 1) SWEEPY = .TRUE. 
CASE ('xz','XZ')
   IF (MOD(G%NTIMESTEPS,2) .EQ. 0) SWEEPX = .TRUE. 
   IF (MOD(G%NTIMESTEPS,2) .EQ. 1) SWEEPZ = .TRUE. 
CASE ('yz','YZ')
   IF (MOD(G%NTIMESTEPS,2) .EQ. 0) SWEEPY = .TRUE. 
   IF (MOD(G%NTIMESTEPS,2) .EQ. 1) SWEEPZ = .TRUE. 

CASE DEFAULT

   IF (NCELLZ .GT. 1 .AND. NCELLX .EQ. 1 .AND. NCELLY .EQ. 1) SWEEPZ = .TRUE.  !1D Z
   IF (NCELLX .GT. 1 .AND. NCELLZ .EQ. 1 .AND. NCELLY .EQ. 1) SWEEPX = .TRUE.  !1D X
   IF (NCELLY .GT. 1 .AND. NCELLX .EQ. 1 .AND. NCELLZ .EQ. 1) SWEEPY = .TRUE.  !1D Y

   IF (NCELLZ .GT. 1 .AND. NCELLX .GT. 1 .AND. NCELLY .EQ. 1) THEN !2D XZ
      IF (MOD(G%NTIMESTEPS,2) .EQ. 0) SWEEPZ = .TRUE. 
      IF (MOD(G%NTIMESTEPS,2) .EQ. 1) SWEEPX = .TRUE. 
   ENDIF

   IF (NCELLZ .GT. 1 .AND. NCELLY .GT. 1 .AND. NCELLX .EQ. 1) THEN !2D YZ
      IF (MOD(G%NTIMESTEPS,2) .EQ. 0) SWEEPZ = .TRUE. 
      IF (MOD(G%NTIMESTEPS,2) .EQ. 1) SWEEPY = .TRUE. 
   ENDIF

   IF (NCELLY .GT. 1 .AND. NCELLX .GT. 1 .AND. NCELLZ .EQ. 1) THEN !2D XY
      IF (MOD(G%NTIMESTEPS,2) .EQ. 0) SWEEPX = .TRUE. 
      IF (MOD(G%NTIMESTEPS,2) .EQ. 1) SWEEPY = .TRUE. 
   ENDIF

   IF (NCELLX .GT. 1 .AND. NCELLY .GT. 1) THEN !3D
      IF (MOD(G%NTIMESTEPS,3) .EQ. 2) SWEEPZ = .TRUE. 
      IF (MOD(G%NTIMESTEPS,3) .EQ. 0) SWEEPX = .TRUE. 
      IF (MOD(G%NTIMESTEPS,3) .EQ. 1) SWEEPY = .TRUE. 
   ENDIF

END SELECT

! *****************************************************************************
END SUBROUTINE SET_SWEEP_DIRECTION
! *****************************************************************************
