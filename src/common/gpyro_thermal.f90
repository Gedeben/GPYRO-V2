...
!================== IF IMPOSED TEMERATURE =======================!

IF (GPG%FIX_DOMAIN_TEMPERATURE) THEN
      CALL GET_CPU_TIME(TMID1)
      G%TPN(:,:,:) = G%TP(:,:,:) + GPG%BETA(IMESH) * DTIME / 60D0
      CALL GET_CPU_TIME(TMID2)
      GPG%TUSED(3) = GPG%TUSED(3) + TMID2 - TMID1 !
...
!=============== BOUNDARY CONDITIONS =======================!
CALL BC_FOR_SOLID_ENTHALPY_SOLVER(IMESH,G%QSBC,AP,AB,AT,AE,AW,AN,AS,B)
CALL GET_CPU_TIME(TMID2)
...
IF (G%ORIENTATION_FILE_EXISTS) THEN
   !$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(IX,IY,IZ,ISPEC)&
   !$OMP SHARED(G,SPROP,NCELLX,NCELLY,NCELLZ,NSSPEC) COLLAPSE(3)
   DO IY = 1, NCELLY
   DO IX = 1, NCELLX
   DO IZ = 1, NCELLZ
...
DO IZ = 1, NCELLZ
   ...
   G%CPS  (IZ,IX,IY) = 0D0
   G%HI (:,IZ,IX,IY) = 0D0
   IF (M%IMASK(IZ,IX,IY)) THEN
      G%CPS  (IZ,IX,IY) = 1D0 ! Only to not divide by zero
      CYCLE
   ENDIF
   DO ISPEC = 1, NSSPEC
      ! Heat capacity
...
SELECT CASE(TRIM(CQUANTITY))
   CASE('KOC')
      ...
      IF (NCELLZ .GT. 1) THEN
         ...
         !$OMP SHARED(G,GAMPZ,NCELLX,NCELLY,NCELLZ) COLLAPSE(3)
         DO IY = 1, NCELLY
         DO IX = 1, NCELLX
         DO IZ = 1, NCELLZ
            IF (M%IMASK(IZ,IX,IY)) THEN; GAMPZ(IZ,IX,IY)=0D0; CYCLE; ENDIF
      ...
      IF (NCELLX .GT. 1) THEN
         !$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(IX,IY,IZ)&
         !$OMP SHARED(G,GAMPX,NCELLX,NCELLY,NCELLZ) COLLAPSE(3)
         DO IY = 1, NCELLY
         DO IX = 1, NCELLX
         DO IZ = 1, NCELLZ
            IF (M%IMASK(IZ,IX,IY)) THEN; GAMPX(IZ,IX,IY)=0D0; CYCLE; ENDIF
      ...
      IF (NCELLY .GT. 1) THEN
         !$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(IX,IY,IZ)&
         !$OMP SHARED(G,GAMPY,NCELLX,NCELLY,NCELLZ) COLLAPSE(3)
         DO IY = 1, NCELLY
         DO IX = 1, NCELLX
         DO IZ = 1, NCELLZ
            IF (M%IMASK(IZ,IX,IY)) THEN; GAMPY(IZ,IX,IY)=0D0; CYCLE; ENDIF
...
IF (NCELLZ .GT. 1) THEN
   ...
   !$OMP SHARED(G,M,NSPEC,NCELLX,NCELLY,NCELLZ) COLLAPSE(3)
   DO IY = 1, NCELLY
   DO IX = 1, NCELLX
   DO IZ = 1, NCELLZ
   IF (M%IMASK(IZ,IX,IY)) CYCLE
   IF (.NOT. G%NEEDSBCT(IZ,IX,IY)) THEN
      DO ISPEC = 1, NSPEC
         ...
         ATHI = G%KOCHIT(ISPEC,IZ,IX,IY) * M%GZT(IZ,IX,IY)
         G%SHYI(IZ,IX,IY)=G%SHYI(IZ,IX,IY)+ ATHI * DYIT
      ENDDO
   ENDIF
   IF (.NOT. G%NEEDSBCB(IZ,IX,IY)) THEN
      DO ISPEC = 1, NSPEC
         ...
         ABHI = G%KOCHIB(ISPEC,IZ,IX,IY) * M%GZB(IZ,IX,IY)
         G%SHYI(IZ,IX,IY)=G%SHYI(IZ,IX,IY) + ABHI * DYIB
      ENDDO
   ENDIF
   ...
   DO IZ = 1, NCELLZ
      ...
      IF (.NOT. G%NEEDSBCW(IZ,IX,IY)) THEN
         DO ISPEC = 1, NSPEC
            ...
            AWHI= G%KOCHIW(ISPEC,IZ,IX,IY) * M%GXW(IZ,IX,IY)
            G%SHYI(IZ,IX,IY)=G%SHYI(IZ,IX,IY)+ AWHI * DYIW
         ENDDO
      ENDIF
      ...
      IF (.NOT. G%NEEDSBCE(IZ,IX,IY)) THEN
         DO ISPEC = 1, NSPEC
            ...
            AEHI = G%KOCHIE(ISPEC,IZ,IX,IY)* M%GXE(IZ,IX,IY)
            G%SHYI(IZ,IX,IY)=G%SHYI(IZ,IX,IY) + AEHI * DYIE
         ENDDO
      ENDIF
   ENDDO
   ENDDO 
   ...
   DO IZ = 1, NCELLZ
      ...
      IF (.NOT. G%NEEDSBCS(IZ,IX,IY)) THEN
         DO ISPEC = 1, NSPEC
            ...
            ASHI = G%KOCHIS(ISPEC,IZ,IX,IY)* M%GYS(IZ,IX,IY)
            G%SHYI(IZ,IX,IY)=G%SHYI(IZ,IX,IY) + ASHI * DYIS
         ENDDO
      ENDIF
      ...
      IF (.NOT. G%NEEDSBCN(IZ,IX,IY)) THEN
         DO ISPEC = 1, NSPEC
            ...
            ANHI = G%KOCHIN(ISPEC,IZ,IX,IY)* M%GYN(IZ,IX,IY)
            G%SHYI(IZ,IX,IY)=G%SHYI(IZ,IX,IY) + ANHI * DYIN
         ENDDO
      ENDIF
   ENDDO
   ENDDO 
...
! *****************************************************************************
SUBROUTINE GAS_SOLID_THERMAL_EXCHANGE(IMESH,DTIME)
! *****************************************************************************

INTEGER, INTENT(IN) :: IMESH
REAL(EB), INTENT(IN) :: DTIME
INTEGER :: IZ,IX,IY,IRXN,IOR
INTEGER :: NCELLZ, NCELLY, NCELLX, ICOUNT
REAL(EB), POINTER, DIMENSION(:,:,:) :: MDOTPPB, FLUX
TYPE (GPYRO_MESH_TYPE), POINTER :: M
...
IF (GPG%SOLVE_GAS_ENERGY .AND. (.NOT. GPG%THERMAL_EQUILIBRIUM)) THEN
   ...
   DO IX = 1, NCELLX
   DO IZ = 1, NCELLZ
      G%QSG(IZ,IX,IY)= G%HCV(IZ,IX,IY)*(G%TGN(IZ,IX,IY)-G%TP(IZ,IX,IY))
   ENDDO
   ENDDO
   ENDDO
   !$OMP END PARALLEL DO

ELSEIF (GPG%THERMAL_EQUILIBRIUM ) THEN
   IF (GPG%SOLVE_PRESSURE) THEN !Mass flux calcd at cell 'top':
      MDOTPPB => G%MDOTPPDARCYB
   ELSE !(Instantaneous gas relase)
      MDOTPPB => G%MDOTPPZB
   ENDIF
   ...
   DO IX = 1, NCELLX
      ...
      IF (GPG%FULL_QSG) THEN !Detailed way of calculating QSG
         ...
         !$OMP PARALLEL DO SCHEDULE(STATIC)
         DO IZ = NCELLZ-1, 1, -1
            G%QSG(IZ,IX,IY)= G%QSG(IZ,IX,IY) + MDOTPPB(IZ,IX,IY) * GPROP%CPG * (G%TP(IZ+1,IX,IY)-G%TP(IZ,IX,IY)) / M%DZT(IZ,IX,IY)
         ENDDO
         !$OMP END PARALLEL DO
         ...
         !$OMP PARALLEL DO SCHEDULE(STATIC)
         DO IZ = 1, NCELLZ-1
            FLUX(IZ,IX,IY) = G%PSIRGDB(IZ,IX,IY) * GPROP%CPG * (G%TP(IZ+1,IX,IY)-G%TP(IZ,IX,IY)) / M%DZT(IZ,IX,IY) 
         ENDDO
         !$OMP END PARALLEL DO

         !$OMP PARALLEL DO SCHEDULE(STATIC)
         DO IZ = 1, NCELLZ-1
            G%QSG(IZ,IX,IY) = G%QSG(IZ,IX,IY) - (FLUX(IZ+1,IX,IY) - FLUX (IZ,IX,IY)) / M%DZT(IZ,IX,IY)
         ENDDO
         !$OMP END PARALLEL DO
      ...
      ELSE !Default way of calculating QSG
         !$OMP PARALLEL DO SCHEDULE(STATIC)
         DO IZ = 1, NCELLZ-1
               G%QSG(IZ,IX,IY)= MDOTPPB(IZ,IX,IY) * GPROP%CPG * (G%TP(IZ+1,IX,IY)-G%TP(IZ,IX,IY)) / M%DZB(IZ,IX,IY)
         ENDDO
         !$OMP END PARALLEL DO
   ...
   DO ICOUNT = 1, NGPYRO_FACES_NEEDING_BCS(IMESH)
      ...
      IF (ABS(G%MDOTPPZB(IZ,IX,IY))  .GT. GPG%EPS) THEN
         G%QSG(IZ,IX,IY) = MDOTPPB(IZ,IX,IY) * GPROP%CPG * &
                           (BOUNDARYS%GPBC(ICOUNT)%TINFG-G%TP(IZ,IX,IY)) / M%DZB(IZ,IX,IY)
      ENDIF
   ENDDO 
   !$OMP END PARALLEL DO
...
ENDIF
! *****************************************************************************
END SUBROUTINE GAS_SOLID_THERMAL_EXCHANGE
! *****************************************************************************

! *****************************************************************************
SUBROUTINE CHEMICAL_SOURCE_TERM
! *****************************************************************************

INTEGER :: ISPEC,IZ,IX,IY,IRXN
INTEGER :: NCELLX, NCELLY, NCELLZ, NSPEC, NRXN
REAL(EB):: DH
TYPE (GPYRO_MESH_TYPE), POINTER :: M
...
! formation and destruction of solid-phase species due to reactions (Variation of sensible enthalpy)
...
DO IZ = 1, NCELLZ
   ...
   DO ISPEC = 1, NSPEC
      G%QSC(IZ,IX,IY) = G%QSC(IZ,IX,IY) + G%SOMEGA(3,ISPEC,IZ,IX,IY)* G%HI(ISPEC,IZ,IX,IY)
   ENDDO
ENDDO
ENDDO
ENDDO
...
DO IRXN = 1, NRXN
   ...
   DO IZ = 1, NCELLZ
      ...
      G%QSC(IZ,IX,IY) = G%QSC(IZ,IX,IY) - G%OMEGASFBK(IRXN,IZ,IX,IY) * RXN(IRXN)%DHS
   ENDDO
   ENDDO
   ENDDO
   !$OMP END PARALLEL DO
...
DO IRXN = 1, NRXN
   ...
   DO IZ = 1, NCELLZ
      IF ( M%IMASK(IZ,IX,IY)                ) CYCLE
      IF (.NOT. G%IS_REACTING(IRXN,IZ,IX,IY)) CYCLE
      G%QSC(IZ,IX,IY) = G%QSC(IZ,IX,IY) - G%OMEGASFGK(IRXN,IZ,IX,IY) * RXN(IRXN)%DHV
   ENDDO
   ENDDO
   ENDDO
   !$OMP END PARALLEL DO
...
IF (.NOT. GPG%CONSTANT_DHVOL) THEN
   IF (GPG%THERMAL_EQUILIBRIUM .OR. (.NOT. GPG%SOLVE_GAS_ENERGY)) THEN
      ...
      DO IRXN = 1, NRXN
         ...
         G%QSC(IZ,IX,IY) = G%QSC(IZ,IX,IY) - G%OMEGASFGK(IRXN,IZ,IX,IY) * DH
      ENDDO
      ENDDO
      ENDDO
      ENDDO
...
! *****************************************************************************
END SUBROUTINE CHEMICAL_SOURCE_TERM
! *****************************************************************************
...
CALL GAS_SOLID_THERMAL_EXCHANGE(IMESH,DTIME)
CALL CHEMICAL_SOURCE_TERM()

!$OMP PARALLEL DO SCHEDULE(STATIC) COLLAPSE(2)
DO IY = 1, NCELLY
DO IX = 1, NCELLX
DO IZ = 1, NCELLZ
   G%SHNET(IZ,IX,IY)= G%QSBC(IZ,IX,IY) ! Add the boundary condition in the source term
   IF (M%IMASK(IZ,IX,IY)) CYCLE
   G%SHNET(IZ,IX,IY)= G%SHNET(IZ,IX,IY) + G%QSC(IZ,IX,IY)- G%QSG(IZ,IX,IY)
   ! The term omega_gas*h_p shows up when the gas-phase mass conservation equation
...
DO IZ = 1, NCELLZ
   ...
   AP0(IZ,IX,IY) = G%MASS_N(IZ,IX,IY) / DTIME
   AP (IZ,IX,IY) = AP0(IZ,IX,IY)
   B  (IZ,IX,IY) = AP0(IZ,IX,IY) * G%HP(IZ,IX,IY)
   IF (NCELLZ .GT. 1) THEN
      AB (IZ,IX,IY) = G%KOCB(IZ,IX,IY) * M%GZB(IZ,IX,IY)
      AT (IZ,IX,IY) = G%KOCT(IZ,IX,IY) * M%GZT(IZ,IX,IY)
   ENDIF
   IF (NCELLX .GT. 1) THEN
      AE (IZ,IX,IY) = G%KOCE(IZ,IX,IY) * M%GXE(IZ,IX,IY)
      AW (IZ,IX,IY) = G%KOCW(IZ,IX,IY) * M%GXW(IZ,IX,IY)
   ENDIF
   IF (NCELLY .GT. 1) THEN
      AN (IZ,IX,IY) = G%KOCN(IZ,IX,IY) * M%GYN(IZ,IX,IY)
      AS (IZ,IX,IY) = G%KOCS(IZ,IX,IY) * M%GYS(IZ,IX,IY)
   ENDIF
ENDDO
ENDDO
ENDDO
...
DO IZ = 1, NCELLZ
   IF (M%IMASK(IZ,IX,IY)) THEN
      ...
      CYCLE
   ENDIF
   A11W(IZ,IX,IY) = G%K_TENSOR(IZ,IX,IY,1,1) * M%SXW(IZ,IX,IY) / ( M%DXW(IZ,IX,IY) * G%CPS(IZ,IX,IY) )
   A12W(IZ,IX,IY) = G%K_TENSOR(IZ,IX,IY,1,2) * M%SXW(IZ,IX,IY) / ( M%DYS(IZ,IX,IY) * G%CPS(IZ,IX,IY) )
   A13W(IZ,IX,IY) = G%K_TENSOR(IZ,IX,IY,1,3) * M%SXW(IZ,IX,IY) / ( M%DZT(IZ,IX,IY) * G%CPS(IZ,IX,IY) )

   A11E(IZ,IX,IY) = A11W(IZ,IX,IY)
   A12E(IZ,IX,IY) = A12W(IZ,IX,IY)
   A13E(IZ,IX,IY) = A13W(IZ,IX,IY)

   A21S(IZ,IX,IY) = G%K_TENSOR(IZ,IX,IY,2,1) * M%SYS(IZ,IX,IY) / ( M%DXW(IZ,IX,IY) * G%CPS(IZ,IX,IY) )
   A22S(IZ,IX,IY) = G%K_TENSOR(IZ,IX,IY,2,2) * M%SYS(IZ,IX,IY) / ( M%DYS(IZ,IX,IY) * G%CPS(IZ,IX,IY) )
   A23S(IZ,IX,IY) = G%K_TENSOR(IZ,IX,IY,2,3) * M%SYS(IZ,IX,IY) / ( M%DZT(IZ,IX,IY) * G%CPS(IZ,IX,IY) )

   A21N(IZ,IX,IY) = A21S(IZ,IX,IY)
   A22N(IZ,IX,IY) = A22S(IZ,IX,IY)
   A23N(IZ,IX,IY) = A23S(IZ,IX,IY)

   A31T(IZ,IX,IY) = G%K_TENSOR(IZ,IX,IY,3,1) * M%DXDY(IZ,IX,IY) / ( M%DXW(IZ,IX,IY) * G%CPS(IZ,IX,IY) )
   A32T(IZ,IX,IY) = G%K_TENSOR(IZ,IX,IY,3,2) * M%DXDY(IZ,IX,IY) / ( M%DYS(IZ,IX,IY) * G%CPS(IZ,IX,IY) )
   A33T(IZ,IX,IY) = G%K_TENSOR(IZ,IX,IY,3,3) * M%DXDY(IZ,IX,IY) / ( M%DZT(IZ,IX,IY) * G%CPS(IZ,IX,IY) )
...
! *****************************************************************************
SUBROUTINE BC_FOR_SOLID_ENTHALPY_SOLVER(IMESH,QSBC,AP,AB,AT,AE,AW,AN,AS,B)
! *****************************************************************************
INTEGER, INTENT(IN   ) :: IMESH
REAL(EB),INTENT(OUT), DIMENSION(:,:,:) :: QSBC
REAL(EB),INTENT(INOUT), DIMENSION(:,:,:) :: AP,AB,AT,AE,AW,AN,AS,B
INTEGER  :: ICOUNT
INTEGER  :: NCELLX, NCELLY, NCELLZ
REAL(EB), DIMENSION(:,:,:), POINTER :: QSBC_THREAD
TYPE (GPYRO_MESH_TYPE), POINTER :: M
TYPE (GPYRO_BOUNDARYS_INFORMATION), POINTER :: BOUNDARYS
...
QSBC = 0D0
! Boundary conditions are considered as sources term, and is added to QSBC,
! For the coefficient of TDMA solver, A_IOR = 0 in the direction of the boundary.

!$OMP PARALLEL & 
!$OMP PRIVATE(ICOUNT, QSBC_THREAD) & 
!$OMP SHARED(NGPYRO_FACES_NEEDING_BCS,BOUNDARYS,G, AP, AT, AB, AE, AW, AN, AS, B,IMESH)
ALLOCATE(QSBC_THREAD(1:NCELLZ,1:NCELLX,1:NCELLY)); QSBC_THREAD = 0D0
!$OMP DO SCHEDULE(STATIC)
DO ICOUNT = 1, NGPYRO_FACES_NEEDING_BCS(IMESH)
   CALL BC_FOR_HALF_CELLS(IMESH,ICOUNT,QSBC_THREAD,QSBC,AP,AT,AB,AE,AW,AN,AS,B)

   !IF(BOUNDARYS%COMPLETE_CELL_AT_BC(ICOUNT)) THEN
   !   CALL BC_FOR_FULL_CELLS(IMESH,ICOUNT,QSBC_THREAD,AP,AT,AB,AE,AW,AN,AS,B)
   !ELSE
   !   CALL BC_FOR_HALF_CELLS(IMESH,ICOUNT,QSBC_THREAD,QSBC,AP,AT,AB,AE,AW,AN,AS,B)
   !ENDIF
END DO
...
!$OMP CRITICAL
QSBC = QSBC + QSBC_THREAD
DEALLOCATE(QSBC_THREAD)
!$OMP END CRITICAL
!$OMP END PARALLEL
...
! *****************************************************************************
SUBROUTINE BC_FOR_FULL_CELLS(IMESH,ICOUNT,QSBC_THREAD,AP,AT,AB,AE,AW,AN,AS,B)
! *****************************************************************************
REAL(EB), INTENT(INOUT), DIMENSION(:,:,:), POINTER :: QSBC_THREAD
REAL(EB), INTENT(INOUT), DIMENSION(:,:,:),TARGET :: AP, AB, AT, AE, AW, AN, AS, B
INTEGER, INTENT(IN) :: IMESH,ICOUNT
...
SELECT CASE(IOR)
   CASE( 3)
      ...
      IX_G        =  IX 
      IY_G        =  IY
      DELTA_IOR   =  M%DZN (IZ  ,IX  ,IY  )
      DS_IOR      =  M%DXDY(IZ  ,IX  ,IY  )
   ...
   CASE( 1)
      ...
      IX_G        =  IX+1
      IY_G        =  IY
      DELTA_IOR   =  M%DX  (IZ  ,IX  ,IY  )
      DS_IOR      =  M%SXE (IZ  ,IX  ,IY  )
   ...
   CASE( 2)
      ...
      IX_G        =  IX
      IY_G        =  IY+1
      DELTA_IOR   =  M%DY  (IZ  ,IX  ,IY  )
      DS_IOR      =  M%SYN (IZ  ,IX  ,IY  )
...
IF (BOUNDARYS%GPBC(ICOUNT)%TFIXED .GE. 0D0) THEN
   BOUNDARYS%GPBC(ICOUNT)%HFIXED = 0D0
   DO ISPEC = 1, NSSPEC
      BOUNDARYS%GPBC(ICOUNT)%HFIXED = BOUNDARYS%GPBC(ICOUNT)%HFIXED + &
                                 G%YIN(ISPEC,IZ,IX,IY) * HOFT(ISPEC,BOUNDARYS%GPBC(ICOUNT)%TFIXED)
   ENDDO
   A_IOR_GOHST        = -1D0
   AP(IZ_G,IX_G,IY_G) =  1D0
   B (IZ_G,IX_G,IY_G) = 2 * BOUNDARYS%GPBC(ICOUNT)%HFIXED
   RETURN
ENDIF
...
QCONV1 = BOUNDARYS%GPBC(ICOUNT)%HC0 *(BOUNDARYS%GPBC(ICOUNT)%TINF - GPG%TDATUM)
!QCONV1 = BOUNDARYS%GPBC(ICOUNT)%HC0 *(BOUNDARYS%GPBC(ICOUNT)%TINF - BOUNDARYS%GPBC(ICOUNT)%T_SURFACE_OLD)
B (IZ_G,IX_G,IY_G) = B (IZ_G,IX_G,IY_G) + QCONV1 * DS_IOR

HTC_BC = HTC_BC + BOUNDARYS%GPBC(ICOUNT)%HC0
...
IF (BOUNDARYS%GPBC(ICOUNT)%RERAD) THEN 
   D_T4=(BOUNDARYS%GPBC(ICOUNT)%T_SURFACE_OLD**4-BOUNDARYS%GPBC(ICOUNT)%TINF**4)
   !D_T4 = G%TP(IZ,IX,IY)**4-BOUNDARYS%GPBC(ICOUNT)%TINF**4
   BOUNDARYS%GPBC(ICOUNT)%QENET = BOUNDARYS%GPBC(ICOUNT)%QENET - EMIS * SIGMA * D_T4
ENDIF


! Incoming Radiation Flux
BOUNDARYS%GPBC(ICOUNT)%QENET = BOUNDARYS%GPBC(ICOUNT)%QENET + EMIS*BOUNDARYS%GPBC(ICOUNT)%QE

KAPPA = G%KAPPA(IZ,IX,IY) !kappa at surface cell

IF ((BOUNDARYS%GPBC(ICOUNT)%QENET .GT. 0D0) .AND. (KAPPA .LT. 1D6)) THEN !In depth Radiation
   CALL IN_DEPTH_RADIATION_ABSORPTION(BOUNDARYS%GPBC(ICOUNT)%QENET,QSBC_THREAD,IZ,IX,IY,IOR,IMESH)

ELSE
   B(IZ_G,IX_G,IY_G) = B(IZ_G,IX_G,IY_G) + BOUNDARYS%GPBC(ICOUNT)%QENET * DS_IOR
ENDIF
...
ENDDO
G%CPS(IZ_G,IX_G,IY_G)= G%CPS(IZ,IX,IY)
...
! *****************************************************************************
SUBROUTINE BC_FOR_HALF_CELLS(IMESH,ICOUNT,QSBC_THREAD,QSBC,AP,AT,AB,AE,AW,AN,AS,B)
! *****************************************************************************
REAL(EB), INTENT(INOUT), DIMENSION(:,:,:), POINTER :: QSBC_THREAD
REAL(EB), INTENT(INOUT), DIMENSION(:,:,:) :: QSBC
REAL(EB), INTENT(INOUT), DIMENSION(:,:,:) :: AP, AB, AT, AE, AW, AN, AS, B
INTEGER, INTENT(IN) :: IMESH,ICOUNT
INTEGER :: IZ,IX,IY,IOR
...
IF (ABS(IOR) == 3) DELTA_IOR = M%DZN(IZ,IX,IY)
IF (ABS(IOR) == 1) DELTA_IOR = M%DX (IZ,IX,IY)
IF (ABS(IOR) == 2) DELTA_IOR = M%DY (IZ,IX,IY)
...
IF (BOUNDARYS%GPBC(ICOUNT)%TFIXED .GE. 0D0) THEN
   ...
   CALL SET_BC_FIXED_VALUE(AP(IZ,IX,IY),AT(IZ,IX,IY),AB(IZ,IX,IY),AE(IZ,IX,IY),AW(IZ,IX,IY),&
                           AN(IZ,IX,IY),AS(IZ,IX,IY),B(IZ,IX,IY),BOUNDARYS%GPBC(ICOUNT)%HFIXED)
   RETURN
ENDIF
...
! Convection Part
QCONV = BOUNDARYS%GPBC(ICOUNT)%HC0 * (BOUNDARYS%GPBC(ICOUNT)%TINF - G%TP(IZ,IX,IY))
QSBC(IZ,IX,IY) = QSBC(IZ,IX,IY) + QCONV / DELTA_IOR

! Radiation Part
EMIS  = CALC_EMIS(IZ,IX,IY)
BOUNDARYS%GPBC(ICOUNT)%QENET = EMIS * BOUNDARYS%GPBC(ICOUNT)%QE

IF (BOUNDARYS%GPBC(ICOUNT)%RERAD) THEN 
   BOUNDARYS%GPBC(ICOUNT)%QENET = BOUNDARYS%GPBC(ICOUNT)%QENET &
                           - EMIS*5.67D-8 *(G%TP(IZ,IX,IY)**4D0-BOUNDARYS%GPBC(ICOUNT)%TINF**4D0)
ENDIF

   KAPPA = G%KAPPA(IZ,IX,IY) !kappa at surface cell
   IF ((BOUNDARYS%GPBC(ICOUNT)%QENET .GT. 0.) .AND. (KAPPA .LT. 1D6)) THEN !In depth Radiation
      CALL IN_DEPTH_RADIATION_ABSORPTION(BOUNDARYS%GPBC(ICOUNT)%QENET,QSBC_THREAD,IZ,IX,IY,IOR,IMESH)
   ELSE! No In_depth_radiation
      QSBC(IZ,IX,IY) = QSBC(IZ,IX,IY) + BOUNDARYS%GPBC(ICOUNT)%QENET / DELTA_IOR
   ENDIF
...
DO IZ = 1, NCELLZ
   IF (M%IMASK(IZ,IX,IY)) CYCLE
   B (IZ,IX,IY) = B (IZ,IX,IY) + SHNET(IZ,IX,IY)*M%DV(IZ,IX,IY)
   IF (SHNET(IZ,IX,IY) .GE. 0D0) THEN
      !B (IZ,IX,IY) = B (IZ,IX,IY) + SHNET(IZ,IX,IY)*M%DV(IZ,IX,IY)
      G%SHP(IZ,IX,IY) = SHNET(IZ,IX,IY)
   ELSE
      !write(0,*) "I am here", IZ
      !AP(IZ,IX,IY) = AP(IZ,IX,IY) - SHNET(IZ,IX,IY)*M%DV(IZ,IX,IY) / G%HPN(IZ,IX,IY)
...
DO ICOUNT = 1, NGPYRO_FACES_NEEDING_BCS(IMESH)
   ...
   IF(BOUNDARYS%COMPLETE_CELL_AT_BC(ICOUNT)) THEN
      ...
      END SELECT
      !WRITE(0,*) "BC", IZ, IZ_G
      BOUNDARYS%GPBC(ICOUNT)%T_SURFACE = G%TPN(IZ,IX,IY)
      !BOUNDARYS%GPBC(ICOUNT)%T_SURFACE = 0.5D0 *(G%TPN(IZ,IX,IY) + G%TPN(IZ_G,IX_G,IY_G))
   ELSE
      BOUNDARYS%GPBC(ICOUNT)%T_SURFACE = G%TPN(IZ,IX,IY)
   ENDIF
END DO
...
DO IX = 1, NCELLX
DO IZ = 1, NCELLZ  
   IF (.NOT.IEEE_IS_FINITE(G%TPN(IZ,IX,IY)) .OR. G%TPN(IZ,IX,IY).GT. 1D10 .OR. G%TPN(IZ,IX,IY).LT.-1D10) THEN
      LOCAL_NAN = .TRUE.
   ENDIF
   IF (.NOT.IEEE_IS_FINITE(G%HPN(IZ,IX,IY)) .OR. G%HPN(IZ,IX,IY).GT. 1D15 .OR. G%HPN(IZ,IX,IY).LT. -1D15) THEN
      LOCAL_NAN = .TRUE.
   ENDIF
