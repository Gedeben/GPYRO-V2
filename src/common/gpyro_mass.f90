...
IF ((.NOT. CI%CONVERGED_YIS(0))) THEN
   ...
   IF (M%DEFORMATION .OR. G%NTIMESTEPS .EQ. 1) THEN
...
CALL GET_CPU_TIME(TMID2) ; GPG%TUSED(10) = GPG%TUSED(10) + TMID2 - TMID3
...
! RRT=Z*exp(-TA/T) * TIME_INTEGRATED_MASS_PRODUCED / DZ plus O2 dependency, if any
!$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(IX,IY,IZ,IRXN,MULT) &
!$OMP SHARED(G,M,SPROP,TS,RXN,GPG,NCELLX,NCELLY,NCELLZ,NRXN) COLLAPSE(3)
DO IY = 1, NCELLY
DO IX = 1, NCELLX
DO IZ = 1, NCELLZ
   ...
   DO IRXN = 1, NRXN
      ...
      IF (TS(IZ,IX,IY) .GE. RXN(IRXN)%TCRIT) THEN
         ...
         G%RRT(IRXN,IZ,IX,IY) = MULT*RXN(IRXN)%Z*EXP(-RXN(IRXN)%E/(R0*TS(IZ,IX,IY))) &
                               * G%TIME_INTEGRATED_MASS_PRODUCED(RXN(IRXN)%IFROM,IZ,IX,IY)/M%DV(IZ,IX,IY)
      ENDIF
      IF (ABS(RXN(IRXN)%ORDERO2) .GT. EPSILON_FB) THEN
...
! Update TIME_INTEGRATED_MASS_PRODUCED
IF (.NOT. GPG%CONVENTIONAL_RXN_ORDER) THEN
   !$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(IX,IY,IZ,ISPEC)&
   !$OMP SHARED(G,M,DTIME,NCELLX,NCELLY,NCELLZ) COLLAPSE(3)
   DO IY = 1, NCELLY
   DO IX = 1, NCELLX
   DO IZ = 1, NCELLZ
   IF (.NOT. G%IS_REACTING(0,IZ,IX,IY)) THEN
      G%TIME_INTEGRATED_MASS_PRODUCEDN(:,IZ,IX,IY)=G%TIME_INTEGRATED_MASS_PRODUCED(:,IZ,IX,IY) 
      CYCLE
   ENDIF
   DO ISPEC = 1, NSSPEC
      G%TIME_INTEGRATED_MASS_PRODUCEDN(ISPEC,IZ,IX,IY)=G%TIME_INTEGRATED_MASS_PRODUCED(ISPEC,IZ,IX,IY) &
                                                      + G%SOMEGA(1,ISPEC,IZ,IX,IY)*M%DV(IZ,IX,IY)*DTIME
   ENDDO
   ENDDO
   ENDDO
   ENDDO
...
DO IZ = 1, NCELLZ
DO ISPEC = 1, NSSPEC
   IF(G%CONSUMED(IZ,IX,IY) .OR. M%IMASK(IZ,IX,IY) .OR. (.NOT. G%IS_REACTING(0,IZ,IX,IY))) CYCLE
   IF (G%TIME_INTEGRATED_MASS_PRODUCED(ISPEC,IZ,IX,IY) .GT. 0D0) THEN
      G%UNREACTEDNESS(ISPEC,IZ,IX,IY) = G%MASS_I(ISPEC,IZ,IX,IY) / G%TIME_INTEGRATED_MASS_PRODUCED(ISPEC,IZ,IX,IY)
...
!$OMP PRIVATE(IX, IY, IZ, IRXN, MI, N, OMN, IKM, OMALPHA, ALPHA) &
...
DO IRXN = 1, NRXN
  ! In some specific cases do nothing
   IF(G%CONSUMED(IZ,IX,IY) .OR. M%IMASK(IZ,IX,IY) .OR. (.NOT. G%IS_REACTING(IRXN,IZ,IX,IY))) THEN
   ...
   IF (.NOT. (MI .GT. -0.00001 .AND. MI .LT. 0.00001)) THEN
      ALPHA = MAX(1D-10,ALPHA)
      G%RRY(IRXN,IZ,IX,IY) = G%RRY(IRXN,IZ,IX,IY) * ALPHA ** MI
   ENDIF
...
IF (.NOT. G%CONV_INFO%CONVERGED_YJG(0) .AND. GPROP%NHGRXN .GT. 0) THEN
   ...
   DO IZ = 1, NCELLZ
      ...
      DO IRXN = 1, GPROP%NHGRXN
         ...
         IF (ABS(P -1D0) .LE. EPSILON_EB) G%HGRR(IRXN,IZ,IX,IY) = CA
         IF (ABS(P -1D0) .GT. EPSILON_EB) G%HGRR(IRXN,IZ,IX,IY) = CA**P
            
         IF (ABS(Q -1D0) .LE. EPSILON_EB) G%HGRR(IRXN,IZ,IX,IY) = G%HGRR(IRXN,IZ,IX,IY) * CB
         IF (ABS(Q -1D0) .GT. EPSILON_EB) G%HGRR(IRXN,IZ,IX,IY) = G%HGRR(IRXN,IZ,IX,IY) * CB**Q
            
         IF (ABS(B) .GT. EPSILON_EB) THEN
         ...
         DO IGSPEC = 1, GPROP%NGSPEC
            IF (ABS(GPROP%HGYIELDS(IGSPEC,IRXN)) .LT. EPSILON_FB) CYCLE
            IF (GPROP%HGYIELDS(IGSPEC,IRXN) .GT. EPSILON_FB) THEN !Formation
...
REAL(EB)    :: MASS
REAL(EB), DIMENSION(SPROP%NSSPEC) :: MASS_I
...
IF (.NOT. G%IS_REACTING(0,IZ,IX,IY)) THEN
   ...
   IF (.NOT. GPG%IS_DENSITY_DEPENDENT_ON_TEMPERATURE) RETURN
   SUM_YI_O_RHOI =0D0
   DO ISPEC = 1, NSSPEC
      IF (G%YIN(ISPEC,IZ,IX,IY) .LE. EPSILON_EB) CYCLE 
      RHOI  = RHOOFT(ISPEC,T)
...
! Calculate the Rho*Dz (The total MASS in the cell)
G%MASS_N(IZ,IX,IY) = -(DTIME * G%GOMEGA(3,0,IZ,IX,IY) * M%DV(IZ,IX,IY)) + G%MASS(IZ,IX,IY)
MASS=G%MASS_N(IZ,IX,IY)
! check if mass remains in the cell
NO_MORE_MASS = .FALSE.
IF ( MASS .LT. EPSILON_EB) NO_MORE_MASS = .TRUE.

SUM_YI_O_RHOI  = 0D0
YI_SUM         = 0D0 ! Help to check Final convergence (coverged if YI_SUM=1)

DO ISPEC = 1, NSSPEC
   ! CLACLUATE Rho*Yi*Dz (The mass of species i)
   G%MASS_IN(ISPEC,IZ,IX,IY) = DTIME *G%SOMEGA(3,ISPEC,IZ,IX,IY) * M%DV(IZ,IX,IY) &
                              + G%MASS_I(ISPEC,IZ,IX,IY)
   MASS_I(ISPEC) = G%MASS_IN(ISPEC,IZ,IX,IY)

   ! Check that no surestimated mass was destroyed.
   IF (G%MASS_IN(ISPEC,IZ,IX,IY) .LT. - EPSILON_EB) THEN
      ! If it occurs, we will call adjust ADJUST_SPECIES_IN_CELL
      CONVERGED = .FALSE.
      RETURN
   ELSEIF (G%MASS_IN(ISPEC,IZ,IX,IY) .LT. 0D0 .AND.  G%MASS_IN(ISPEC,IZ,IX,IY) .GT. - EPSILON_EB )THEN
      ! This is if the numerical approximation give a very small numerical negatif value
      G%MASS_IN(ISPEC,IZ,IX,IY) = 0D0 
   ENDIF

   IF (NO_MORE_MASS) CYCLE

   ! Determine the mass fraction of species i (Y_i)

    G%YIN(ISPEC,IZ,IX,IY) = G%MASS_IN(ISPEC,IZ,IX,IY) / G%MASS_N(IZ,IX,IY)

   IF (.NOT. IEEE_IS_FINITE(G%YIN(ISPEC,IZ,IX,IY))) THEN
      GPG%NAN = .TRUE.
   ENDIF
   ...
   IF (G%YIN(ISPEC,IZ,IX,IY) .LE. EPSILON_EB) CYCLE ! No more species Yi in the cell
   RHOI  = RHOOFT(ISPEC,T)
   SUM_YI_O_RHOI  = SUM_YI_O_RHOI + G%YIN(ISPEC,IZ,IX,IY)/RHOI
...
MASS_DESTROYED = -TIME_STEP * G%SOMEGA(3, SPECIES, IZ, IX, IY) * M%DV(IZ, IX, IY)
MASS_AVAILABLE = G%MASS_I(SPECIES, IZ, IX, IY)
...
G%SOMEGA(3, SPECIES, IZ, IX, IY) = -MASS_AVAILABLE / (TIME_STEP * M%DV(IZ, IX, IY))
...
G%SOMEGA(2, SPECIES, IZ, IX, IY) = OVERESTIMATED_ISPEC_DESTRUCTION_RATE &
                               - EXCESS_MASS_DESTROYED / (TIME_STEP * M%DV(IZ, IX, IY))
...
!The Solide mass fraction used is the new one that is alredy calculated
YI => G%YIN
...
DO IZ = 1, NCELLZ
   ...
   SUM_YI_O_RHOSI = 0
   DO ISPEC = 1, NSSPEC
      IF (ABS(YI(ISPEC,IZ,IX,IY)) .LT. EPSILON_FB) CYCLE ! No more species Yi in the cell
...
IF (ABS(SPROP%NR(ISPEC)) .LT. EPSILON_FB) THEN
