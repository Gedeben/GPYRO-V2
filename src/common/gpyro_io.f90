...
REAL(EB) :: TAMB,GX,GZ,GY,DT0,TREF,P0,ALPHA,ALPHA_YIS,ALPHA_YJG,ALPHA_H,ALPHA_HG,ALPHA_P,TMPTOL, &
            ...
            GPYRO_TO_GPYRO_TOLERANCE, CONSTANT_HM0, EPS_YJG,EPS_YIS,DTMIN_KILL

REAL(EB) :: ADD_TO_OBST_XB, ADD_TO_OBST_X, ADD_TO_OBST_Y, ADD_TO_OBST_Z,OBST_SCALING_FACTOR! DEAD parameters
...
INTEGER, DIMENSION(1:1000) :: PROFILE_ISKIP,POINT_QUANTITY_INDEX,PROFILE_QUANTITY_INDEX,SMOKEVIEW_QUANTITY_INDEX,&
                              POINT_IMESH,PROFILE_IMESH,SMOKEVIEW_IMESH
...
REAL(EB), DIMENSION(100,6) :: HCR ! DEAD parameter
LOGICAL, DIMENSION(1:100) :: HALF_CELLS_AT_BC
...
NAMELIST /GPYRO_GENERAL/ P0,TAMB,GX,GZ,GY,DT0,TREF,NTDMA_ITERATIONS, &
         ...
         FDS_MATL_VER,CONV_DIFF_SCHEME, TORTUOSITY_FACTOR, USE_TORTUOSITY_FACTOR_FOR_FLUX, USE_CONSTANT_HM0, CONSTANT_HM0, &
...
NAMELIST /GPYRO_OUTPUT/ CASENAME,N_POINT_QUANTITIES,N_PROFILE_QUANTITIES, N_SMOKEVIEW_QUANTITIES, &
         DTDUMP_GA, DTDUMP_POINT, DTDUMP_PROFILE, DTDUMP_SMOKEVIEW, &
         POINT_IMESH, PROFILE_IMESH, SMOKEVIEW_IMESH, &
         POINT_QUANTITY, PROFILE_QUANTITY, SMOKEVIEW_QUANTITY, &
         POINT_QUANTITY_INDEX, PROFILE_QUANTITY_INDEX, SMOKEVIEW_QUANTITY_INDEX, &
         POINT_Z,POINT_X,POINT_Y, &
         PROFILE_DIRECTION, PROFILE_COORD1,PROFILE_COORD2,PROFILE_ISKIP, &
         SMOKEVIEW_PLANE,SMOKEVIEW_LOCATION, &
         DUMP_DETAILED_CONVERGENCE, &
         TMP_REDUCED_DTDUMP,REDUCED_DTDUMP, &
         DTMIN_KILL ! Option mooved to &Gpyro General
...
NAMELIST /GPYRO_GEOM/ NMESH, NOBST, ZDIM, NCELLZ, XDIM, NCELLX, YDIM, NCELLY, HALF_CELLS_AT_BC,&
                      ...
                      IMESH, Z1, Z2, X1, X2, Y1, Y2, ICNUM, SURF_IDX2D, &
                      OFFSETZ, OFFSETX, OFFSETY,HCR ! DEAD OPTION
...
!GPG%DTMIN_KILL                   = DTMIN_KILL  ! Now is set in GPYRO_GENERAL
GPG%DUMP_DETAILED_CONVERGENCE      = DUMP_DETAILED_CONVERGENCE
...
ALLOCATE(GPG%HALF_CELLS_AT_BC(1:NMESH    )) 
...
GPG%HALF_CELLS_AT_BC(1:NMESH    ) = HALF_CELLS_AT_BC(1:NMESH    )
...
!******************************************************************************
CASENAME                = 'gpyro'
N_POINT_QUANTITIES      = 0
N_PROFILE_QUANTITIES    = 0
...
INTEGER :: ISPEC,IRXN,IOBST,IC,ISURF
CHARACTER(4) :: FOUR
CHARACTER(2) :: TWO
CHARACTER(300) :: FN
CHARACTER(200) :: STR1 
...
IF (.NOT. LOPEN) THEN
   ...
   WRITE(LUDOTOUT,'(1X,A)')      'Gpyro V2.0'
...
ELSE 
   ...
   WRITE(LUDOTOUT,*)
   WRITE(LUDOTOUT,90) ' ******************************************************************************* '
   WRITE(LUDOTOUT,98) ' Simulation time: ', TI, ' s     Timestep: ', GPG%DT, ' s     # of iterations: ', G%CONV_INFO%ITER_ALL
   WRITE(LUDOTOUT,99) ' Minimum temperature: ', GPG%MINTMP-273.15D0, ' deg C         Maximum temperature: ', GPG%MAXTMP - 273.15D0, ' deg C'
...
ENDIF
CALL GET_CPU_TIME(TMID2) ; GPG%TUSED(41)= GPG%TUSED(41) +TMID2-TMID1
...
GPG%TUSED(40) = GPG%TUSED(40) + TEND - TSTART 

CONTAINS

!******************************************************************************
SUBROUTINE DUMP_POINT_QUANTITIES(ICASE,IMESH)
!******************************************************************************
...
DO I = 1, GPG%N_POINT_QUANTITIES
   ...
   SELECT CASE(GPG%POINT_QUANTITY(I))
      ...
      CASE ('N_ITERATIONS')
         TEMPVAR = REAL(G%CONV_INFO%ITER_ALL,EB)
         PTR => TEMPVAR
...
INTEGER :: I, J, K, N, IZ, IX, IY, ISPEC, IRXN, IOS, STR_SIZE
...
DO I = 1, GPG%N_PROFILE_QUANTITIES
   ...
   SELECT CASE (GPG%PROFILE_QUANTITY(I))
      ...
      CASE ('S') !Source term
         IF (PROF_DIR .EQ. 'z') PTR => G%SHNET(: ,IX,IY)
         IF (PROF_DIR .EQ. 'x') PTR => G%SHNET(IZ,: ,IY)
         IF (PROF_DIR .EQ. 'y') PTR => G%SHNET(IZ,IX,: )
      ...
      CASE ('SHYI')
         IF (PROF_DIR .EQ. 'z') PTR => G%SHYI(: ,IX,IY)
         IF (PROF_DIR .EQ. 'x') PTR => G%SHYI(IZ,: ,IY)
         IF (PROF_DIR .EQ. 'y') PTR => G%SHYI(IZ,IX,: )
   ...
   STR_SIZE = CEILING(REAL(N) / REAL(GPG%PROFILE_ISKIP(I))) + 1
   BUFFER => G%WORK_DUMP(I)%BUFFER
   LINE   => G%WORK_DUMP(I)%LINE
...
CHARACTER(2)  :: TWO
INTEGER IZ,IX,IY,IBAR,JBAR,KBAR,ONE_INTEGER,N,I,J,K,I1,I2,J1,J2,K1,K2,LUSF,KI, &
         NMESHES,ISPEC,IRXN,ICOUNT,IOS

LOGICAL :: LOPEN,IMASKVALTODUMP
...
IF (.NOT. G%SMOKEVIEW_FILE_OPENED_ALREADY) THEN
   ...
   ! this, set IMASKVALTODUMP to TRUE.  But to dump the geometry itself, useful for 
   ! diagnostic purposes, set IMASKVALTODUMP to FALSE. 
   IMASKVALTODUMP = .TRUE.
   ...
   ! Now that .smv file is open and initialized, do the same 
   ! for the slice files
   DO N=1,GPG%N_SMOKEVIEW_QUANTITIES
      ...
      SELECT CASE (GPG%SMOKEVIEW_QUANTITY(N))
         ...
         CASE('THERMAL_CONDUCTIVITY_Z')
            LONGNAME  = 'z thermal conductivity'
            SHORTNAME = 'kz'
            UNITS     = 'W/m-K'

         CASE('THERMAL_CONDUCTIVITY_X')
            LONGNAME  = 'x thermal conductivity'
            SHORTNAME = 'kx'
            UNITS     = 'W/m-K'

         CASE('THERMAL_CONDUCTIVITY_Y')
            LONGNAME  = 'y thermal conductivity'
            SHORTNAME = 'ky'
            UNITS     = 'W/m-K'
         ...
         CASE('SHP') 
            LONGNAME = '+ solid enthalpy source term'
            SHORTNAME = 'SHP'
            UNITS     = 'W/m3'              
         ...
         CASE('SHGP') 
            LONGNAME = '+ gas enthalpy source term'
            SHORTNAME = 'SHGP'
            UNITS     = 'W/m3'              
...
DO N=1,GPG%N_SMOKEVIEW_QUANTITIES
   ...
   SELECT CASE(GPG%SMOKEVIEW_QUANTITY(N))
      ...
      CASE DEFAULT
         WRITE(0,'(A,A,A)') 'Error, Smokeview quantity ', GPG%SMOKEVIEW_QUANTITY(N), ' not found.'
   ...
   DO I=I1,I2
      ...
      QQ(I,J,K) = REAL(QQTEMP(I,J,K), kind=4)
   ...
   INQUIRE(UNIT=LUSMV+N,OPENED=LOPEN)
   IF (LOPEN) FLUSH(LUSMV+N) 
...
CHARACTER(300) :: FN
INTEGER :: IOS
REAL(EB) :: TOTAL_TIME, SOLID_ENTHALPY_TIME, SOLID_SPECIES_TIME
REAL(EB) :: DUMP_TIME, TIME_OTHER
!CHARACTER(2) :: TWO


!WRITE(TWO, '(I2.2)') ICASE
FN = TRIM(GPG%CASENAME) // '_timing'  // '.csv'
OPEN(UNIT=LUTIME, FILE=FN, FORM='FORMATTED', STATUS='REPLACE', IOSTAT=IOS)


TOTAL_TIME              = GPG%TUSED(0 )
SOLID_ENTHALPY_TIME     = GPG%TUSED(1 )
SOLID_SPECIES_TIME      = GPG%TUSED(10)
DUMP_TIME               = GPG%TUSED(40)

IF (TOTAL_TIME .LE. 0D0) RETURN
TIME_OTHER= TOTAL_TIME-(SOLID_ENTHALPY_TIME+SOLID_SPECIES_TIME+GPG%TUSED(30)+DUMP_TIME)

! ==========================
! SECTION 1 :  Global timing
! ==========================
WRITE(LUTIME,102) '============== TIMING SUMMARY ==============='
WRITE(LUTIME,100) 'Total CPU Time (s):', TOTAL_TIME
WRITE(LUTIME,101) 'Subroutine', 'Time (s)', 'CPU (%)'
WRITE(LUTIME,102) '---------------------------------------------'
WRITE(LUTIME,103) 'SOLID_SPECIES_SOLVER'       , SOLID_SPECIES_TIME , 100.0 * SOLID_SPECIES_TIME  / TOTAL_TIME
WRITE(LUTIME,103) 'SOLID_ENTHALPY_SOLVER'      , SOLID_ENTHALPY_TIME, 100.0 * SOLID_ENTHALPY_TIME / TOTAL_TIME
WRITE(LUTIME,103) 'GAS_EQUATION'               , GPG%TUSED(30)      , 100.0 * GPG%TUSED(30) / TOTAL_TIME
WRITE(LUTIME,103) 'DUMP_ROUTINES'              , DUMP_TIME          , 100.0 * DUMP_TIME / TOTAL_TIME
WRITE(LUTIME,103) 'ALL OTHERS'                 , TIME_OTHER         , 100.0 * TIME_OTHER/ TOTAL_TIME
WRITE(LUTIME,102) '---------------------------------------------'
...
IF (SOLID_SPECIES_TIME .GT. 0D0) THEN
GPG%TUSED(15)=SOLID_SPECIES_TIME-SUM(GPG%TUSED(11:14))

   WRITE(LUTIME,102) '=========== SOLID SPECIES SOLVER ============'  
   WRITE(LUTIME,101) 'Subroutine', 'Time (s)', 'CPU (%)'  
   WRITE(LUTIME,102) '---------------------------------------------'  
   WRITE(LUTIME,103) 'REACTION_RATE'          , GPG%TUSED(11), 100.0 * GPG%TUSED(11) / SOLID_SPECIES_TIME
   WRITE(LUTIME,103) 'SPECIES_SOURCE_TERMS'   , GPG%TUSED(12), 100.0 * GPG%TUSED(12) / SOLID_SPECIES_TIME  
   WRITE(LUTIME,103) 'SOLVE_YI_AND_DENISTY'   , GPG%TUSED(13), 100.0 * GPG%TUSED(13) / SOLID_SPECIES_TIME  
   WRITE(LUTIME,103) 'MESH_DEFORMATION'       , GPG%TUSED(14), 100.0 * GPG%TUSED(14) / SOLID_SPECIES_TIME
   WRITE(LUTIME,103) 'Others'                 , GPG%TUSED(15), 100.0 * GPG%TUSED(15) / SOLID_SPECIES_TIME  
   WRITE(LUTIME,102) '---------------------------------------------'  
   WRITE(LUTIME,103) 'TOTAL:'          , SOLID_SPECIES_TIME, 100.0
   WRITE(LUTIME,*  ) " " 
   WRITE(LUTIME,*  ) " " 
ENDIF

IF (SOLID_ENTHALPY_TIME .GT. 0) THEN
   GPG%TUSED(8)=SOLID_ENTHALPY_TIME-SUM(GPG%TUSED(2:7))

   WRITE(LUTIME,102) '========== SOLID ENTHALPY SOLVER ============'  
   WRITE(LUTIME,101) 'Subroutine', 'Time (s)', 'CPU (%)'  
   WRITE(LUTIME,102) '---------------------------------------------' 
   WRITE(LUTIME,103) 'THERMAL_PROPERTIES'     , GPG%TUSED(9), 100.0 * GPG%TUSED(9) / SOLID_ENTHALPY_TIME  
   WRITE(LUTIME,103) 'ENERGIE SOURCE TERMS'   , GPG%TUSED(2), 100.0 * GPG%TUSED(2) / SOLID_ENTHALPY_TIME  
   WRITE(LUTIME,103) 'GET TDMA COEFF'         , GPG%TUSED(3), 100.0 * GPG%TUSED(3) / SOLID_ENTHALPY_TIME  
   WRITE(LUTIME,103) 'SHYI CORRECTION'        , GPG%TUSED(4), 100.0 * GPG%TUSED(4) / SOLID_ENTHALPY_TIME  
   WRITE(LUTIME,103) 'BC HANDELING'           , GPG%TUSED(5), 100.0 * GPG%TUSED(5) / SOLID_ENTHALPY_TIME  
   WRITE(LUTIME,103) 'SOLVE TDMA'             , GPG%TUSED(6), 100.0 * GPG%TUSED(6) / SOLID_ENTHALPY_TIME  
   WRITE(LUTIME,103) 'T FROM H'               , GPG%TUSED(7), 100.0 * GPG%TUSED(7) / SOLID_ENTHALPY_TIME  
   WRITE(LUTIME,103) 'Others'                 , GPG%TUSED(8), 100.0 * GPG%TUSED(8) / SOLID_ENTHALPY_TIME  
   WRITE(LUTIME,102) '---------------------------------------------'  
   WRITE(LUTIME,103) 'TOTAL:'          , SOLID_ENTHALPY_TIME, 100.0
...
IF (DUMP_TIME .GT. 0D0) THEN 
   GPG%TUSED(44)=DUMP_TIME-SUM(GPG%TUSED(41:43))

   WRITE(LUTIME,102) '============== DUMP ROUTINES ================'  
   WRITE(LUTIME,101) 'Subroutine', 'Time (s)', 'CPU (%)'  
   WRITE(LUTIME,102) '---------------------------------------------'  
   WRITE(LUTIME,103) 'POINT DATA'             , GPG%TUSED(41), 100.0 * GPG%TUSED(41) / DUMP_TIME
   WRITE(LUTIME,103) 'SMOKEVIEW FILE'         , GPG%TUSED(42), 100.0 * GPG%TUSED(42) / DUMP_TIME  
   WRITE(LUTIME,103) 'PROFILE DATA'           , GPG%TUSED(43), 100.0 * GPG%TUSED(43) / DUMP_TIME  
   WRITE(LUTIME,103) 'Others'                 , GPG%TUSED(44), 100.0 * GPG%TUSED(44) / DUMP_TIME  
   WRITE(LUTIME,102) '---------------------------------------------'  
   WRITE(LUTIME,103) 'TOTAL:'          , DUMP_TIME, 100.0
   
   WRITE(LUTIME,*  ) " " 
   WRITE(LUTIME,*  ) " "
ENDIF

IF (TIME_OTHER .GT. 0D0) THEN 
   GPG%TUSED(56)=TIME_OTHER-SUM(GPG%TUSED(51:55))

   WRITE(LUTIME,102) '================ ALL OTHERS ================='  
   WRITE(LUTIME,101) 'Subroutine', 'Time (s)', 'CPU (%)'  
   WRITE(LUTIME,102) '---------------------------------------------'
   WRITE(LUTIME,103) 'READ INPUT FILE'                  , GPG%TUSED(51), 100.0 * GPG%TUSED(51) / TIME_OTHER
   WRITE(LUTIME,103) 'INITIALISATION'                   , GPG%TUSED(52), 100.0 * GPG%TUSED(52) / TIME_OTHER  
   WRITE(LUTIME,103) 'GET BC TIME STEP'                 , GPG%TUSED(54), 100.0 * GPG%TUSED(54) / TIME_OTHER   
   WRITE(LUTIME,103) 'UPDATE SOLUTION TIME STEP'        , GPG%TUSED(55), 100.0 * GPG%TUSED(55) / TIME_OTHER   
   !WRITE(LUTIME,103) 'DEALLOCATE MEMORY'                , GPG%TUSED(53), 100.0 * GPG%TUSED(53) / TIME_OTHER  
   WRITE(LUTIME,103) 'Others'                           , GPG%TUSED(56), 100.0 * GPG%TUSED(56) / TIME_OTHER  
   WRITE(LUTIME,102) '---------------------------------------------'  
   WRITE(LUTIME,103) 'TOTAL:'          , TIME_OTHER, 100.0
ENDIF
...
100 FORMAT(A, F8.2)
101 FORMAT(A25, A10, A10)
102 FORMAT(A)
103 FORMAT(A25, F10.2, F8.2)
...
INTEGER :: IOS, I, N
LOGICAL :: LOPEN
!Close point quantites   
INQUIRE(UNIT=LUPOINT,OPENED=LOPEN)
IF (LOPEN) THEN
    CLOSE(LUPOINT, IOSTAT=IOS)   
   IF (IOS .NE. 0)  WRITE(0,'(A,I3)') "Error while closing point quantities "
ENDIF

INQUIRE(UNIT=LUCSV,OPENED=LOPEN)
IF (LOPEN)  CLOSE(LUCSV)

! Close profile quantites
DO I = 1, GPG%N_PROFILE_QUANTITIES
   INQUIRE(UNIT=LUPROF+I,OPENED=LOPEN)
   IF (LOPEN) THEN
       CLOSE(LUPROF+I, IOSTAT=IOS)   
      IF (IOS .NE. 0)  WRITE(0,'(A)') "Error while closing profile quantities "
   ENDIF
ENDDO

! Close smokeview quantites
DO N=1,GPG%N_SMOKEVIEW_QUANTITIES
   INQUIRE(UNIT=LUSMV+N,OPENED=LOPEN)
   IF (LOPEN) THEN
      CLOSE(LUSMV+N, IOSTAT=IOS)  
   IF (IOS .NE. 0)  WRITE(0,'(A)') "Error while closing smokeview quantites "
   ENDIF
ENDDO
...
IF (GO) CALL DUMP_TIMINGS ! Dump final cpu time statistics
